...
// pdf-lib con fuentes estándar solo dibuja Latin-1. Los caracteres invisibles que se
// cuelan al copiar y pegar (espacios de ancho cero, guiones largos, comillas curvas)
// tumban la generación, así que se limpian antes de escribir.
function limpio(v) {
  ...
  t = t.replace(/[\u2018\u2019\u201A\u201B]/g, "'")
       ...
       .replace(/[\u2026]/g, '...')
       .replace(/\u00A0/g, ' ');
  // Lo que siga sin caber en Latin-1 se cambia por su letra sin acento, o se quita
...
async function estadoDeCuenta(datos, opciones) {
  ...
  const der = (t, x, w, yy, f, s, c) => {
    page.drawText(limpio(t), { x: x + w - f.widthOfTextAtSize(t, s), y: yy, size: s, font: f, color: c || TINTA });
  };
  ...
  function nuevaPagina() {
    ...
    y -= 20;
    if (pagina === 1) {
      page.drawText(limpio('ESTADO DE CUENTA'), { x: M, y, size: 13, font: neg, color: TINTA });
      der(o.fecha || '', M, ANCHO, y, reg, 9, SUAVE);
      y -= 16;
      page.drawText(limpio(datos.proveedor || ''), { x: M, y, size: 11, font: neg, color: TINTA });
      y -= 26;
    } else { y -= 6; }
  }
  ...
  function titulo(t, sub, color) {
    espacio(56);
    page.drawText(limpio(t), { x: M, y, size: 9.5, font: neg, color: color || TINTA });
    y -= 11;
    if (sub) {
      page.drawText(limpio(sub), { x: M, y, size: 7.5, font: reg, color: SUAVE });
      y -= 10;
    }
  ...
  function tabla(cols, filas) {
    if (!filas.length) {
      page.drawText(limpio('Nada por aqui.'), { x: M, y, size: 8, font: reg, color: SUAVE });
      y -= 18; return;
    }
    let x = M;
    cols.forEach(c => {
    ...
    filas.forEach(f => {
      ...
      cols.forEach((c, i) => {
        ...
        else page.drawText(limpio(t), { x: x, y, size: 8, font: f._neg ? neg : reg, color: f._color || TINTA });
        x += c.w;
      });
      ...
      if (f._nota) {
        ...
        cortar(f._nota, reg, 6.5, ANCHO - 20).slice(0, 2).forEach(l => {
          page.drawText(limpio(l), { x: M + 10, y, size: 6.5, font: reg, color: SUAVE });
          y -= 8;
        });
        y -= 2;
      }
  ...
  function totalBloque(etiqueta, valor) {
    ...
    page.drawText(limpio(etiqueta), { x: M, y, size: 7.5, font: neg, color: SUAVE });
  ...
  const cajas = [
    ...
    { t: 'CON CLIENTE', v: String(totalVendidas), s: 'te faltan por entregar', c: TINTA },
  ...
  cajas.forEach((c, i) => {
    ...
    page.drawText(limpio(c.t), { x: x + 9, y: y - 15, size: 6.5, font: neg, color: SUAVE });
    page.drawText(limpio(c.v), { x: x + 9, y: y - 32, size: 14, font: neg, color: c.c });
    page.drawText(limpio(c.s), { x: x + 9, y: y - 41, size: 6, font: reg, color: SUAVE });
  });
  y -= 66;
  ...
  if ((datos.vendidas || []).length) {
    ...
    Object.keys(porFolio).forEach(k => {
      ...
      page.drawText(limpio(v.folio), { x: M + 10, y, size: 9, font: neg, color: TINTA });
      if (v.cliente) {
        page.drawText(limpio(v.cliente), { x: M + 10 + neg.widthOfTextAtSize(v.folio, 9) + 10, y,
      ...
      v.piezas.forEach(p2 => {
        const etiqueta = '· ' + p2.piezas + '  ' + p2.producto +
                         (p2.material ? ' · ' + p2.material : '');
        page.drawText(limpio(etiqueta), { x: M + 14, y, size: 8, font: reg, color: TINTA });
        // De qué pedido sale esa pieza: sin esto el proveedor no sabe a cuál orden pertenece
        if (p2.pedido) {
          der('Pedido ' + p2.pedido, M, ANCHO - 10, y, reg, 7, SUAVE);
        }
        y -= 11;
        if (p2.detalle) {
          cortar(p2.detalle, reg, 6.8, ANCHO - 44).forEach(l => {
            page.drawText(limpio(l), { x: M + 24, y, size: 6.8, font: reg, color: SUAVE });
            y -= 8;
          });
        }
      });
      if (v.direccion) {
        ...
        cortar('Entregar en: ' + v.direccion + (v.telefono ? '  ·  Tel. ' + v.telefono : ''),
          ...
          page.drawText(limpio(l), { x: M + 14, y, size: 7, font: reg, color: TINTA });
          y -= 9;
        });
      }
      y -= 6;
  ...
  page.drawText(limpio('TOTAL POR PAGAR'), { x: A4.w - M - 290, y: y - 4, size: 9.5, font: neg, color: TINTA });
  ...
  pdf.getPages().forEach((p, i) => {
    ...
    const tl = limpio(t);
    p.drawText(tl, { x: (A4.w - reg.widthOfTextAtSize(tl, 7)) / 2, y: M - 20, size: 7, font: reg, color: SUAVE });
  });
