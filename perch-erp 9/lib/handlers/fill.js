...
module.exports = async (req, res) => {
  try {
    ...
    // La columna puede llamarse distinto en cada hoja: se aceptan alternativas
    const candidatos = [match.col].concat(match.colAlt || []);
    let iMatch = -1;
    for (const c of candidatos) {
      iMatch = headers.findIndex(h => norm(h) === norm(c));
      if (iMatch !== -1) break;
    }
    if (iMatch === -1) {
      return res.status(400).json({
        error: 'No se encontró la columna "' + match.col + '" en la pestaña ' + cfg.sheetName + '.',
