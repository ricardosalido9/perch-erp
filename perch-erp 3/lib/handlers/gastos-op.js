...
// No son gasto: mueven dinero entre cuentas propias. Pagar la tarjeta no es gasto
// (el gasto fue cuando se usó). Ojo: las comisiones bancarias SÍ son gasto y pasan.
const NO_ES_GASTO = /traspaso|transferencia entre cuentas|entre cuentas|movimiento interno|pago de tarjeta|pago a tarjeta|pago tarjeta|pago de tarjetas/i;
...
module.exports = async (req, res) => {
  try {
    ...
    egr.rows.forEach(r => {
      ...
      const texto = norm(txt(eCon ? r[eCon] : '') + ' ' + txt(eDes ? r[eDes] : '') + ' ' +
                         txt(eSub ? r[eSub] : '') + ' ' + txt(eCat ? r[eCat] : ''));
      if (NO_ES_GASTO.test(texto)) return;
      const mes = Math.floor(d / 100) % 100;
      // Estas etiquetas viven en la columna Subcategoría. Si no coincide ahí,
      // se prueba con Categoría por si algún renglón quedó clasificado al revés.
      const sub = txt(eSub ? r[eSub] : '');
      const cat = txt(eCat ? r[eCat] : '');
      const oficial = catsNorm[norm(sub)] || catsNorm[norm(cat)];
      const mov = {
        fila: r._fila, fecha: txt(r[eFec]), monto,
        concepto: txt(eCon ? r[eCon] : '') || txt(eSub ? r[eSub] : ''),
        categoria: txt(eCat ? r[eCat] : ''),
      ...
      if (oficial) {
        ...
        // Lo que no es gasto operativo: costo de venta, impuestos, socios…
        const etq = sub || cat;
        if (!otrasCategorias[etq]) otrasCategorias[etq] = { categoria: etq, monto: 0, n: 0 };
        otrasCategorias[etq].monto += monto;
        otrasCategorias[etq].n++;
      }
    });
    ...
    return res.status(200).json({
      ...
      columnas: {
        subcategoria: eSub || '(NO ENCONTRADA)',
        categoria: eCat || '(NO ENCONTRADA)'
      },
