...
.nota{background:#fbf8f0;border-left:3px solid var(--acc);padding:14px 18px;margin-top:14px}
.nota-t{font-family:Fraunces,Georgia,serif;font-size:15px;font-weight:600;margin-bottom:6px}
.nota-x{font-size:13.5px;line-height:1.6}
.pie{margin-top:34px;font-size:11px;color:var(--muted);border-top:1px solid var(--line);
     padding-top:12px}
.tabla-caja{background:var(--card);border:1px solid var(--line);margin-top:14px;
            overflow-x:auto}
.tabla{border-collapse:collapse;width:100%;font-family:"IBM Plex Mono",monospace;
       font-variant-numeric:tabular-nums;font-size:11px}
.tabla th{font-family:Inter,sans-serif;font-size:9.5px;letter-spacing:.09em;
          text-transform:uppercase;color:var(--muted);font-weight:500;padding:10px 8px;
          text-align:right;border-bottom:1px solid var(--ink);white-space:nowrap}
.tabla th.con,.tabla td.con{text-align:left;position:sticky;left:0;background:var(--card);
          font-family:Inter,sans-serif;font-size:12px;min-width:210px}
.tabla td{padding:7px 8px;text-align:right;white-space:nowrap;border-bottom:1px solid #f4f1e9}
.tabla td.neg{color:var(--mal)}
.tabla td.cierre{border-left:1px solid var(--line)}
.tabla tr.fuerte td{font-weight:600;background:#faf8f2}
.tabla tr.total td{font-weight:600;border-top:1px solid var(--ink);
          border-bottom:1px solid var(--ink);background:#f6f3ec}
@media print{body{background:#fff;padding:0}.wrap{max-width:none}
  .tabla-caja{overflow:visible}.tabla{font-size:8.5px}
  .tabla th.con,.tabla td.con{position:static;min-width:0}
...
// La tabla completa, con los meses como columnas. Es lo que se viene a revisar
// renglón por renglón: el análisis dice qué mirar, la tabla es el estado.
function tabla(t) {
  const filas = t.filas || [];
  if (!filas.length) return '';
  return '<h2>' + esc(t.titulo) + (t.nota ? '<em>' + esc(t.nota) + '</em>' : '') + '</h2>' +
    '<div class="tabla-caja"><table class="tabla"><thead><tr>' +
    (t.columnas || []).map((c, i) =>
      '<th' + (i === 0 ? ' class="con"' : '') + '>' + esc(c) + '</th>').join('') +
    '</tr></thead><tbody>' +
    filas.map(f => {
      const cls = [];
      if (f.fuerte) cls.push('fuerte');
      if (f.nivel === 0) cls.push('total');
      return '<tr class="' + cls.join(' ') + '">' +
        '<td class="con" style="padding-left:' + (8 + (f.nivel || 0) * 12) + 'px">' +
          esc(f.concepto) + '</td>' +
        (f.celdas || []).map((c, i) =>
          '<td class="' + ((typeof c === 'number' && c < 0) ? 'neg' : '') +
          (i === (f.celdas.length - 1) || i === (f.celdas.length - 3) ? ' cierre' : '') + '">' +
          esc(typeof c === 'number' ? (t.formato === 'miles'
              ? (c === 0 ? '\u2014' : Math.round(c / 1000).toLocaleString('en-US'))
              : dinero(c))
            : (c == null ? '' : c)) + '</td>').join('') +
        '</tr>';
    }).join('') + '</tbody></table></div>' +
    (t.pie ? '<div class="s" style="font-size:11px;color:var(--muted);margin-top:6px">' +
      esc(t.pie) + '</div>' : '');
}

// Una nota escrita por una persona. Se ve distinta de todo lo demás a propósito:
// lo que calcula el sistema y lo que opina alguien no pueden confundirse.
function nota(n) {
  if (!n || !n.texto) return '';
  return '<div class="nota">' +
    (n.titulo ? '<div class="nota-t">' + esc(n.titulo) + '</div>' : '') +
    '<div class="nota-x">' + esc(n.texto).replace(/\n/g, '<br>') + '</div>' +
    '</div>';
}
...
// ---- la página ----
function tablero(d) {
  // Las notas se pegan a su sección: se escriben con el nombre de la sección y
  // aparecen justo debajo de ella. La que se llame "general" va hasta arriba.
  const notas = d.notas || {};
  const conNotas = [];
  if (notas.general) conNotas.push({ tipo: 'nota', texto: notas.general, titulo: d.tituloNota || '' });
  (d.bloques || []).forEach(b => {
    conNotas.push(b);
    const clave = b.titulo || '';
    if (clave && notas[clave]) conNotas.push({ tipo: 'nota', texto: notas[clave] });
  });
  d = Object.assign({}, d, { bloques: conNotas });
  ...
  const bloques = (d.bloques || []).map(b => {
    ...
    if (b.tipo === 'tabla') return tabla(b);
    if (b.tipo === 'nota') return nota(b);
