...
const ARCHIVOS = {
  ...
  // Los CFDIs descargados del SAT. Los tres viven en el mismo archivo, cada uno
  // en su pestaña: EMITIDOS, RECIBIDOS y NOMINA. Se dejan como tres variables
  // separadas por si algún día vuelven a partirse en archivos distintos.
  CFDI_EMITIDOS: id('SHEET_CFDI_EMITIDOS', '19uNUj_qIA5UmLQQSRy35cA7t39BIIYKhY88waSitvB4'),
  CFDI_RECIBIDOS:id('SHEET_CFDI_RECIBIDOS','19uNUj_qIA5UmLQQSRy35cA7t39BIIYKhY88waSitvB4'),
  COLABORADORES: id('SHEET_COLABORADORES', '1OE5Pm9eCqlq3CUmt8m8sm0lgL_lTQbBt-PHIKCqoE2M'),
  // El catálogo de cuentas vive en su propio archivo, no en el de Finanzas.
  CATALOGO_CUENTAS: id('SHEET_CATALOGO_CUENTAS', '1cKns85GvnfM3Himu_a9AdwFEenP0ofTu8ou4jyiLNoI'),
  NOMINA:        id('SHEET_NOMINA',        '19uNUj_qIA5UmLQQSRy35cA7t39BIIYKhY88waSitvB4'),
  // Estados financieros. Guarda un renglón por año, mes y concepto en la pestaña
  // EF Datos; la estructura del estado vive en EF Conceptos. El ERP arma la vista
  // desde ahí, así que no depende de que las columnas sean los meses.
  ESTADOS:       id('SHEET_ESTADOS',       '15UYq88YKtLOxjZ-sSKh_z0ae1WZYWnd6PlZlslKBG7s'),
  ...
  // La carpeta donde van las ventas y cotizaciones, con una subcarpeta por mes
  // y dentro una por folio: MAYO / MY9-26 ALEX HERRERA
  // La carpeta donde se guardan cotizaciones, ventas, CSF y comprobantes
  DRIVE_VENTAS:  id('DRIVE_VENTAS',        '1SWWOSte9CluAQ9nZazuw-zlIJTV3et1g'),
  // La carpeta con las fotos de los productos
  DRIVE_FOTOS:   id('DRIVE_FOTOS',         '1GZDB-F_PLc3eMlmgZSRVJApuKpu6DOuh')
};
...
const PESTANAS = {
  ...
  entradas:        E.TAB_ENTRADAS        || 'Entradas',
  ...
  listaPrecios:    E.TAB_LISTA_PRECIOS   || 'Lista de Precios {AAAA}',
  clientes:        E.TAB_CLIENTES        || 'Lista de Clientes',
  proveedores:     E.TAB_PROVEEDORES     || 'Lista de Proveedores',
  // Antes emitidos y recibidos vivían en archivos distintos y las dos pestañas se
  // llamaban igual ("VIGENTES"). Ahora comparten archivo, así que cada lado
  // necesita su propio nombre; si no, las dos leerían la misma hoja.
  cfdiEmitidos:    E.TAB_CFDI_EMITIDOS   || 'EMITIDOS',
  cfdiRecibidos:   E.TAB_CFDI_RECIBIDOS  || 'RECIBIDOS',
  compEmitidos:    E.TAB_COMP_EMITIDOS   || 'COMPLEMENTOS EMITIDOS',
  compRecibidos:   E.TAB_COMP_RECIBIDOS  || 'COMPLEMENTOS RECIBIDOS',
  colaboradores:   E.TAB_COLABORADORES   || 'Nomina',
  // Sin año a propósito: cuando entre 2027 se sigue capturando en la misma
  // pestaña y el histórico no se pierde. El ERP filtra por fecha, no por hoja.
  nomina:          E.TAB_NOMINA          || 'NOMINA',
  ...
  logistica:       E.TAB_LOGISTICA       || 'Logistica',
  // Un juego de pestañas por estado: ER = Estado de Resultados, EF = Estado de
  // Flujo de Efectivo. Cada uno con sus datos y su estructura. El Balance, cuando
  // toque, entra igual con BG Datos y BG Conceptos.
  erDatos:         E.TAB_ER_DATOS        || 'ER Datos',
  erConceptos:     E.TAB_ER_CONCEPTOS    || 'ER Conceptos',
  efDatos:         E.TAB_EF_DATOS        || 'EF Datos',
  efConceptos:     E.TAB_EF_CONCEPTOS    || 'EF Conceptos',
  bgDatos:         E.TAB_BG_DATOS        || 'BG Datos',
  bgConceptos:     E.TAB_BG_CONCEPTOS    || 'BG Conceptos'
};
