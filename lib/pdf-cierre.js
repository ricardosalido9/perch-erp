...
async function reporteMensual(d, o) {
  ...
  function nuevaPagina() {
    ...
    y -= 20;
    if (pagina === 1) {
      txt((o.alcance === 'ventas' ? 'VENTAS DE ' : 'CIERRE DE ') +
          String(o.mes || '').toUpperCase(), M, y, neg, 14, TINTA);
  ...
  // Alcance: el reporte de Ventas trae solo lo comercial, para que las chicas de
  // ventas lo revisen y lo firmen sin cargar con producción. El del Dashboard
  // trae todo.
  const soloComercial = (o.alcance === 'ventas');
  nuevaPagina();
  ...
  // ===== 5. De dónde llegaron =====
  if (!soloComercial && (d.marketing || []).length) {
