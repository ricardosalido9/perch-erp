...
module.exports = async (req, res) => {
  try {
    ...
    // Lo cancelado no cuenta en ningún análisis: es una captura que no existió
    const cStatus = col(H, 'Status');
    const anulada = (r) => cStatus && /cancelad|anulad/i.test(txt(r[cStatus]));
    const esDe = (r, a, m) => {
      if (anulada(r)) return false;
      const d = fechaNum(r[cFec]);
