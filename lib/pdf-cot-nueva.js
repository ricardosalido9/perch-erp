...
async function cotizacionNueva(cot, o) {
  ...
  (cot.productos || []).forEach(p => {
    // El nombre del mueble en su propio renglón y el resto debajo: leer un bloque
    // corrido de texto para saber qué se cotiza cuesta más de lo que parece.
    const nombre = String(p.producto || '').trim();
    const resto = String(p.descripcion || '').replace(
      new RegExp('^' + nombre.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\s*·?\\s*'), '');
    const lineas = (nombre ? [nombre] : []).concat(
      renglones(resto || (nombre ? '' : (p.descripcion || '')), reg, 7.5, COLS.desc - 12))
    ...
    let ly = yTop;
    lineas.forEach((l, i) => {
      // El nombre del mueble va destacado; lo demás es descripción
      const esNombre = (i === 0 && !!nombre);
      txt(l, M + COLS.foto, ly, esNombre ? neg : reg, esNombre ? 8.5 : 7.5, TINTA);
      ly -= esNombre ? 12 : 10;
    });
