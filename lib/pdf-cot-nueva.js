...
async function cotizacionNueva(cot, o) {
  ...
  (cot.productos || []).forEach(p => {
    // Se respeta cómo está escrita la descripción en el catálogo: ahí el nombre
    // va en sus propios renglones y arriba —"Banca COA / TEJIDA"— y debajo el
    // texto. Esos renglones cortos de arriba se dibujan en negritas; el resto,
    // normal. Aplastarlo todo en un párrafo perdía esa jerarquía.
    const nombre = String(p.producto || '').trim();
    const crudo = String(p.descripcion || '');
    const partido = crudo.split(/\r?\n/).map(x => x.trim()).filter(Boolean);

    // Los renglones de arriba que parecen título: cortos y sin punto final
    const esTitulo = (t) => t.length <= 34 && !/[.;,]$/.test(t);
    const titulo = [];
    let i0 = 0;
    while (i0 < partido.length && esTitulo(partido[i0]) && titulo.length < 3) {
      titulo.push(partido[i0]); i0++;
    }
    // Si el catálogo no trae título propio, se usa el nombre del producto
    if (!titulo.length && nombre) titulo.push(nombre);
    else if (titulo.length && nombre &&
             titulo.join(' ').toLowerCase().indexOf(nombre.toLowerCase()) === -1) {
      titulo.unshift(nombre);
    }
    const cuerpo = partido.slice(i0).join(' ');
    const lineas = titulo.concat(
      renglones(cuerpo, reg, 7.5, COLS.desc - 12)).filter(x => String(x).trim());
