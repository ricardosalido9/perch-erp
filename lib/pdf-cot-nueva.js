...
async function cotizacionNueva(cot, o) {
  ...
  if (fontkit) {
    ...
    const cool     = await cargar('Coolvetica-Rg.otf');
    const coolCond = await cargar('Coolvetica-Cond.otf');
    const victoria = await cargar('VictorianOrchid-Light.otf');
    if (cool) reg = cool;
    if (coolCond) neg = coolCond;
    else if (cool) neg = cool;
    tit = victoria || neg;
  }
  ...
  let yt = y;
  const filasTot = [['ANTICIPO', anticipo], ['FINIQUITO', finiquito]];
  if (envio) filasTot.push(['ENVIO', envio]);
  filasTot.push(['TOTAL', granTotal]);
  filasTot.forEach((f, i) => {
    const esTotal = (i === filasTot.length - 1);
