...
async function cotizacionNueva(cot, o) {
  ...
  if (fontkit) {
    const cargar = async (archivo) => {
      ...
      if (!buf) return null;
      try { return await pdf.embedFont(buf, { subset: true }); } catch (e) { return null; }
    };
    // El archivo de Pau usa VictorianOrchid Regular y Book para los rótulos, y
    // Coolvetica Light para el texto. La Light no venía en el paquete, así que
    // por ahora se usa Regular, que es el corte más cercano.
  ...
  txt('P     E     R     C     H', M + 14, barraY + 9, tit, 9, PORTADA, 1.5);
  ...
  function encabezado() {
    page = pdf.addPage([A4.w, A4.h]);
    y = A4.h - M;
    // El logotipo va como texto: son las letras P É R C H separadas, en la
    // tipografía de títulos. Dibujarlo así queda idéntico al diseño y no depende
    // de tener el archivo del logo a la mano.
