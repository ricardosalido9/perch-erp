...
async function cotizacionNueva(cot, o) {
  ...
  (cot.productos || []).forEach(p => {
    ...
    // El precio ya con descuento. Cuando cada renglón lleva un porcentaje distinto
    // se dice cuál, pero debajo y en chico: pegado al precio se leía como si
    // fueran dos cantidades.
    const pctFila = Number(p.descPct) || 0;
    der(money(conDesc), x, COLS.desc2 - 8, yMedio, reg, 8.5, TINTA);
    if (!pctDesc && pctFila) {
      der('-' + pctFila + '%', x, COLS.desc2 - 8, yMedio - 10, reg, 6.5, SUAVE);
    }
    x += COLS.desc2;
  ...
  let yt = y;
  // El orden con el que se lee un pago: primero el envío, que es un concepto
  // aparte; luego el anticipo, que es lo que se cobra ahora; luego el finiquito,
  // que es lo que queda; y al final el total.
  const filasTot = [];
