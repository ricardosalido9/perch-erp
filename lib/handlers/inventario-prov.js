...
module.exports = async (req, res) => {
  try {
    ...
    const pF = col(ped.headers, 'Folio cliente', 'Folio Cliente');
    ...
    ped.rows.forEach(r => {
      ...
      if (!d.pedidos[kp]) d.pedidos[kp] = { pedido: kp, pedidas: 0, entregadas: 0, status: '', costo: null };
    ...
    const filas = Object.keys(inv).map(k => {
      ...
      return {
        ...
        pedidos: Object.keys(d.pedidos).map(x => d.pedidos[x])
          .sort((a, b) => (b.pedidas - b.entregadas) - (a.pedidas - a.entregadas)),
