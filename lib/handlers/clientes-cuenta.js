...
module.exports = async (req, res) => {
  try {
    ...
    let [ven, ing, cxc, ing2] = await Promise.all([
      leer('ventas_registro'), leer('fin_ingresos'), leer('fin_cxc'),
      leerDe(ID_OPERACION, 'INGRESOS')
    ]);
    // Los cobros pueden estar en el histórico de finanzas o en Operación 2026:
    // se revisan los dos, porque el equipo captura en uno u otro según el año.
    let fuentesIngresos = [];
    if (ing.headers.length) fuentesIngresos.push({ nombre: 'histórico', datos: ing });
    if (ing2.headers.length) fuentesIngresos.push({ nombre: 'Operación 2026', datos: ing2 });
    ...
    fuentesIngresos.forEach(fuente => {
      const ing = fuente.datos;
      const iRef = col(ing.headers, 'No. de Referencia', 'Folio', 'Pedido', 'Referencia');
      ...
      const iMet = col(ing.headers, 'Método de cobro', 'Método de pago', 'Metodo de pago', 'Forma de pago');
      ...
      if (iTot) {
        ...
        // Para buscar el folio dentro de un texto hace falta que quede aislado:
        // así "MY6-26" no se confunde con "MY6-260"
        const regexFolios = folios.map(f => ({
          folio: f,
          rx: new RegExp('(^|[^a-z0-9])' + norm(f).replace(/[.*+?^${}()|[\]\\]/g, '\\$&') +
                         '([^a-z0-9]|$)')
        }));
        ...
        ing.rows.forEach(r => {
          ...
          if (!folio) {
            // El folio suele venir dentro de la Descripción o el Concepto
            const texto = ' ' + norm(txt(iDes ? r[iDes] : '') + ' ' + txt(iCon ? r[iCon] : '') +
                                     ' ' + crudo) + ' ';
            const hit = regexFolios.filter(x => x.rx.test(texto))[0];
            folio = hit ? hit.folio : '';
          }
          if (!folio) {
            // Último intento: en cualquier columna del renglón
            const todo = ' ' + norm(ing.headers.map(h => txt(r[h])).join(' ')) + ' ';
            const hit2 = regexFolios.filter(x => x.rx.test(todo))[0];
            folio = hit2 ? hit2.folio : '';
          }
    ...
    return res.status(200).json({
      ...
      fuentesDeIngresos: fuentesIngresos.map(f => ({ nombre: f.nombre, filas: f.datos.rows.length })),
