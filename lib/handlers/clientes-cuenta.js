    ...
    if (ing.headers.length) {
      ...
      if (iTot) {
        ...
        ing.rows.forEach(r => {
          ...
          if (!folio) {
            // Último intento: en cualquier columna del renglón. Hay ingresos donde el
            // folio quedó en una columna suelta o pegado a otro texto.
            const todo = ' ' + norm(ing.headers.map(h => txt(r[h])).join(' ')) + ' ';
            folio = folios.filter(f => todo.indexOf(norm(f)) !== -1)[0] || '';
          }
