  ...
  try {
    ...
    if (ing.headers.length) {
      ...
      if (iTot) {
        ...
        ing.rows.forEach(r => {
          ...
          if (!m) return;
          let folio = txt(iRef ? r[iRef] : '');
          if (!folio || folios.indexOf(folio) === -1) {
            // Se busca el folio dentro del concepto o la descripción
            const texto = ' ' + norm(txt(iCon ? r[iCon] : '') + ' ' + txt(iDes ? r[iDes] : '') + ' ' + folio) + ' ';
    ...
    if (cxc.headers.length) {
      ...
      if (cRef) cxc.rows.forEach(r => {
        ...
        if (!k) return;
        enCxc[k] = {
          total: cTot ? num(r[cTot]) : 0,
          pagado: cPag ? num(r[cPag]) : 0,
          porCobrar: cPor ? num(r[cPor]) : 0
        };
      });
    }
