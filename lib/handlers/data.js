...
module.exports = async (req, res) => {
  try {
    ...
    // Aunque alguien escriba la dirección a mano, no ve un área que no le toca
    if (!core.puedeVerArea(sesion.rol, key)) {
      return res.status(403).json({ error: 'Tu usuario no tiene acceso a esta área.' });
    }
    ...
    // La MISMA regla que usa la escritura: si leer y guardar buscan los
    // encabezados en filas distintas, se ve una cosa y se guarda otra.
    const hr = core._hrIdx ? core._hrIdx(cfg, values)
                           : ((cfg.headerRow && cfg.headerRow > 1) ? (cfg.headerRow - 1) : 0);
    if (values.length <= hr) return res.status(200).json({ connected: true, headers: [], rows: [] });
    let headers = (values[hr] || []).map(String);
    ...
    // Columnas que un renglón tiene que traer para contar como renglón de verdad.
    // Un colaborador sin nombre no es una persona: es un resto de la hoja. Se
    // esconde en vez de salir como renglón en blanco estorbando la lista.
    const cfgArea = core.SHEETS[key] || {};
    const idxRequiere = (cfgArea.requiere || []).map(n => headers.findIndex(h =>
      String(h).trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      === String(n).trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    )).filter(i => i !== -1);
    ...
    for (let i = hr + 1; i < values.length; i++) {
      ...
      if (!hasData) continue;
      if (idxRequiere.length && !idxRequiere.some(c =>
            values[i][c] != null && String(values[i][c]).trim() !== '')) continue;
      const o = { _row: i + 1 };
    ...
    // El comercial no ve costos ni utilidad: las columnas ni siquiera se envían
    const perm = core.permisosDe(sesion.rol);
    if (perm && perm.ocultarColumnas.length) {
      const fuera = perm.ocultarColumnas.map(c => core._norm ? core._norm(c) : String(c).toLowerCase());
      const quitar = headers.filter(h => fuera.indexOf(String(h).toLowerCase().normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '').trim()) !== -1);
      if (quitar.length) {
        headers = headers.filter(h => quitar.indexOf(h) === -1);
        outRows.forEach(r => quitar.forEach(h => { delete r[h]; }));
      }
    }
    return res.status(200).json({
