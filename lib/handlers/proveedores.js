...
module.exports = async (req, res) => {
  try {
    ...
    const ID_EGRESOS = CFG.ARCHIVOS.FINANZAS;
      ...
      if (cfgRev && cfgRev.id) {
        ...
        if (mh.headers.length) {
          ...
          if (mP && mC) mh.rows.forEach(r => {
            ...
            // La nota se guarda SIEMPRE: sirve para entender por qué no cuadra.
            // Y aparte se marca si ya se dio por cerrado con un ajuste manual.
            const cerrado = /ajustes?\s+manual(es)?/i.test(nota);
            // De la nota se sacan los montos: son abonos que no están en EGRESOS
            // (pagos directos del cliente al proveedor, ajustes de IVA, etc.)
            const montos = [];
            const rx = /\$\s*([0-9][0-9,]*(?:\.[0-9]{1,2})?)/g;
            let m2;
            while ((m2 = rx.exec(nota)) !== null) {
              const v = parseFloat(String(m2[1]).replace(/,/g, ''));
              if (!isNaN(v) && v > 0) montos.push(v);
            }
            const segunNico = Math.round(montos.reduce((a, x) => a + x, 0) * 100) / 100;
            const info = { nota: nota, cerrado: cerrado,
                           abonosDeNota: segunNico, cuantosAbonos: montos.length };
            notas[k] = info;
            notas[normPedido(r[mP])] = info;
          });
        }
      }
    ...
    const salida = Object.keys(provs).map(k => {
      ...
      const pedidos = Object.keys(p.pedidos).map(kd => {
        ...
        const info = notas[kLiq] || notas[normPedido(d.pedido)] || null;
        const cerrado = !!(info && info.cerrado);
        const saldoReal = Math.round((d.costo - pg.monto) * 100) / 100;
        // Los abonos que Nico anotó en Mi hoja y no están en EGRESOS
        const abonosNota = (info && info.abonosDeNota) ? info.abonosDeNota : 0;
        const saldoSegunNico = Math.round((saldoReal - abonosNota) * 100) / 100;
        return Object.assign({}, d, {
          // "AJUSTE MANUAL" da el pedido por cerrado: su saldo es cero.
          // Cualquier otra nota solo explica; el saldo se queda como está.
          liquidado: cerrado, notaAjuste: info ? info.nota : '',
          saldoOriginal: saldoReal,
          pagado: pg.monto, pagos: pg.n, detallePagos: pg.detalle || [],
          porPagar: cerrado ? 0 : saldoReal,
          // Lo que de verdad se debe si se cuentan las notas de Nico
          abonosDeNota: abonosNota,
          cuantosAbonos: (info && info.cuantosAbonos) || 0,
          saldoSegunNico: cerrado ? 0 : saldoSegunNico,
      ...
      const conNota = pedidos.filter(x => x.notaAjuste && !x.liquidado);
      const saldoTotal = pedidos.reduce((a, x) => a + x.porPagar, 0);
      // El mismo total, pero descontando lo que Nico anotó en Mi hoja
      const saldoSegunNicoTotal = pedidos.reduce((a, x) =>
        a + (x.saldoSegunNico != null ? x.saldoSegunNico : x.porPagar), 0);
      const abonosDeNotas = pedidos.reduce((a, x) => a + (x.abonosDeNota || 0), 0);
      return {
        ...
        totales: {
          ...
          // Lo cerrado con ajuste manual ya no cuenta en el saldo del proveedor
          ajusteTotal: Math.round(conAjuste.reduce((a, x) => a + (x.saldoOriginal || 0), 0) * 100) / 100,
          porPagar: Math.round(saldoTotal * 100) / 100,
          // Lo que se debe descontando las notas de Nico. Es la cifra que se le
          // dice al proveedor; la de arriba es la que sale de los números.
          porPagarSegunNico: Math.round(saldoSegunNicoTotal * 100) / 100,
          abonosDeNotas: Math.round(abonosDeNotas * 100) / 100
        },
