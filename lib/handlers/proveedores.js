  ...
  try {
    ...
    if (egr.headers.length) {
      ...
      if (gP && gT) egr.rows.forEach(r => {
        // La llave incluye al proveedor: dos proveedores pueden usar el mismo número de
        // pedido, y sin esto los pagos de uno se le cargan al otro.
        if (!norm(r[gP])) return;
        const k = norm(gPr ? r[gPr] : '') + '||' + norm(r[gP]);
    ...
    const salida = Object.keys(provs).map(k => {
      ...
      const pedidos = Object.keys(p.pedidos).map(kd => {
        ...
        const pg = pagos[norm(p.proveedor) + '||' + norm(kd)] || { monto: 0, n: 0, detalle: [] };
