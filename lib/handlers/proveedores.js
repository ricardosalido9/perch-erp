...
// Los pedidos anteriores a este año se migraron incompletos (solo el stock que servía),
// así que su costo está recortado pero sus pagos sí están completos. Restarlos da saldos
// negativos falsos. Por eso el histórico se congela en un saldo inicial por proveedor
// y de aquí en adelante se calcula normal.
const ANIO_CORTE = 2026;
...
module.exports = async (req, res) => {
  try {
    ...
    // Los pagos se leen del EGRESOS del MISMO archivo de Operación, que es donde viven
    // los pedidos. El archivo histórico de finanzas es otro universo y no cruza.
    ...
    if (egr.headers.length) {
      ...
      if (gP && gT) egr.rows.forEach(r => {
        ...
        const aG = gF ? anioDe(r[gF]) : null;
        if (aG !== null && aG < ANIO_CORTE) return;    // pago viejo: ya está en el saldo inicial
        ...
        if (!pagos[k]) pagos[k] = { monto: 0, n: 0, ajustes: 0, nAjustes: 0,
                                    prov: gPr ? txt(r[gPr]) : '', detalle: [] };
        const m = num(r[gT]);
        // Todo renglón de EGRESOS con ese pedido cuenta como pagado. Los ajustes se
        // marcan aparte solo como referencia: también son dinero a favor del proveedor.
        const esAjuste = AJUSTES.test(txt(gD2 ? r[gD2] : '') + ' ' + txt(gC2 ? r[gC2] : ''));
        pagos[k].monto += m;
        pagos[k].n++;
        if (esAjuste) { pagos[k].ajustes += m; pagos[k].nAjustes++; }
        // Máximo 20 pagos por pedido, para no inflar la respuesta
        if (pagos[k].detalle.length < 20) {
          pagos[k].detalle.push({
            fecha: gF ? txt(r[gF]) : '', monto: m, ajuste: esAjuste,
    ...
    ped.rows.forEach(r => {
      ...
      const aP = cFec ? anioDe(r[cFec]) : null;
      if (aP !== null && aP < ANIO_CORTE) return;      // lo viejo va en el saldo inicial
    ...
    // Pagos de este año que corresponden a pedidos anteriores al corte.
    // Cuentan igual: bajan la deuda aunque el pedido ya esté congelado.
    const pagosViejos = {};
    Object.keys(pagos).forEach(k => {
      if (pedidosVigentes[k]) return;
      const partes = k.split('||');
      const prov = partes[1] || '';
      if (!prov) return;
      const p = pagos[k];
      if (!pagosViejos[prov]) pagosViejos[prov] = { monto: 0, n: 0, detalle: [] };
      pagosViejos[prov].monto += p.monto;
      pagosViejos[prov].n += p.n;
      (p.detalle || []).forEach(g => {
        if (pagosViejos[prov].detalle.length < 30) {
          pagosViejos[prov].detalle.push(Object.assign({ pedido: partes[0] }, g));
        }
      });
    });
    ...
    const salida = Object.keys(provs).map(k => {
      ...
      const pedidos = Object.keys(p.pedidos).map(kd => {
        ...
        let pg = { monto: 0, n: 0, ajustes: 0, nAjustes: 0, detalle: [] };
        ...
        return Object.assign({}, d, {
          ...
          ajustes: Math.round((pg.ajustes || 0) * 100) / 100, nAjustes: pg.nAjustes || 0,
      ...
      const aju = pedidos.reduce((a, x) => a + (x.ajustes || 0), 0);
      const ini = saldoInicial[norm(p.proveedor)] || 0;
      const viejos = pagosViejos[norm(p.proveedor)] || { monto: 0, n: 0, detalle: [] };
      return {
        ...
        totales: {
          ...
          costo: tot, pagado: pag, ajustes: Math.round(aju * 100) / 100,
          saldoInicial: Math.round(ini * 100) / 100,
          pagosAPedidosViejos: Math.round(viejos.monto * 100) / 100,
          nPagosViejos: viejos.n,
          detallePagosViejos: viejos.detalle,
          porPagar: Math.round((ini + tot - pag - viejos.monto) * 100) / 100
        },
        ...
        abiertos: {
          ...
          ajustes: suma2(abiertos, 'ajustes'),
    ...
    return res.status(200).json({
      ...
      conSaldosIniciales: !!Object.keys(saldoInicial).length,
