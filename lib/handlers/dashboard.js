...
const MESES = { ene:1, feb:2, mar:3, abr:4, may:5, jun:6, jul:7, ago:8, sep:9, sept:9, oct:10, nov:11, dic:12, enero:1,febrero:2,marzo:3,abril:4,mayo:5,junio:6,julio:7,agosto:8,
...
// Estandariza las respuestas de sí o no. Se usa en marketing, en el funnel y en
// cotizaciones: si cada quien escribe "SI", "Si", "x" o "1", el conteo se parte
// en cuatro categorías que en realidad son la misma.
function siNoGlobal(v) {
  const n = norm(v);
  if (!n || n === 'na' || n === 'n/a' || n === '-' || n === 'sin especificar') return 'Sin especificar';
  if (/^(si|s|x|true|verdadero|1|yes|ya)$/.test(n)) return 'Sí';
  if (/^(no|n|false|falso|0|ninguno|nunca)$/.test(n)) return 'No';
  return txt(v);
}
...
// Palabras que delatan un MATERIAL, no un tipo de mueble. Hay hojas donde la
// columna "Tipo de producto" o "Categoría" quedó llena de maderas, y entonces la
// gráfica de ventas por tipo salía diciendo "Nogal, Encino Negro, Tzalam" en vez
// de "Sillas, Mesas, Bancas".
const SUENA_A_MATERIAL = /nogal|encino|tzalam|parota|caoba|cedro|pino|marmol|mármol|laca|tela|piel|latón|laton|acero|cristal|vidrio|mdf|chapa|rattan|ratán|travertino|onix|ónix|granito|bronce|hierro|mimbre|bejuco|entintad|natural|blanco|negro/i;
const SUENA_A_MUEBLE = /silla|sillon|sillón|mesa|banca|banco|buro|burÓ|burô|taburete|consola|reposet|cama|librero|reposapies|espejo|repisa|cabecera|comedor|escritorio|mueble|decorativ|accesorio|atril|perchero|coffee|lateral|cómoda|comoda|vitrina|barra|puff|sofa|sofá/i;

// Elige la columna que de verdad trae TIPOS DE MUEBLE. Si varias se llaman
// parecido, gana la que más valores de mueble tenga y menos de material.
function colTipoMueble(H, rows, nombres) {
  const cands = H.filter(h => nombres.some(n => norm(h) === norm(n)));
  if (!cands.length) return null;
  let mejor = null, mejorPunto = -1e9;
  cands.forEach(h => {
    let mueble = 0, material = 0, llenos = 0;
    rows.forEach(r => {
      const v = txt(r[h]);
      if (!v) return;
      llenos++;
      if (SUENA_A_MUEBLE.test(v)) mueble++;
      else if (SUENA_A_MATERIAL.test(v)) material++;
    });
    if (!llenos) return;
    // Se premia lo que parece mueble y se castiga fuerte lo que parece material
    const punto = (mueble * 2) - (material * 3) + llenos * 0.01;
    if (punto > mejorPunto) { mejorPunto = punto; mejor = h; }
  });
  return mejor || cands[0];
}
...
module.exports = async (req, res) => {
  try {
    ...
    const cTipo  = colTipoMueble(H, R, ['Tipo de producto', 'Tipo de Producto',
                                        'Categoria producto', 'Categoría', 'Categoria']);
    const cVend  = col(H, 'Vendedor');
    const cCant  = col(H, 'Cantidad', 'Unidades');
    // La VENTA NETA es lo que va al estado de resultados: subtotal menos
    // descuentos más envíos, sin IVA. Si la hoja ya la trae calculada se usa esa;
    // si no, se arma. Antes se tomaba una sola columna y podía ser la bruta, así
    // que el dashboard no cuadraba contra el estado de resultados.
    const cNeta  = col(H, 'Total Venta Neta', 'Venta Neta', 'Total venta neta');
    const cVenta = col(H, 'Total con envio sin impuestos', 'Total con envío sin impuestos');
    const cSub   = col(H, 'Subtotal', 'Subtotales');
    const cDescM = col(H, 'Descuentos', 'Descuento');
    const cEnvM  = col(H, 'Envio', 'Envío', 'Costo Envío', 'Costo Envio');
    const ventaNetaDe = (r) => {
      if (cNeta && txt(r[cNeta]) !== '') return num(r[cNeta]);
      if (cVenta && txt(r[cVenta]) !== '') return num(r[cVenta]);
      // Último recurso: se arma con lo que haya
      const sub = cSub ? num(r[cSub]) : 0;
      const des = cDescM ? num(r[cDescM]) : 0;
      const env = cEnvM ? num(r[cEnvM]) : 0;
      return sub - des + env;
    };
    ...
    const out = ventas.rows.map(r => ({
      ...
      venta: ventaNetaDe(r),
      // La bruta se manda aparte, para poder mostrar el descuento otorgado
      bruta: cSub ? num(r[cSub]) : null,
      desc:  cDescM ? num(r[cDescM]) : 0,
      env:   cEnvM ? num(r[cEnvM]) : 0,
    ...
    try {
      ...
      // "SI", "Si", "sí", "SÍ", "x", "TRUE" y "1" son la misma respuesta. Sin
      // estandarizarlas, cada forma de escribirla salía como una categoría
      // distinta en las gráficas y el conteo quedaba repartido entre cinco.
      const siNo = (v) => {
        const n = norm(v);
        if (!n || n === 'na' || n === 'n/a' || n === '-' || n === 'sin especificar') return 'Sin especificar';
        if (/^(si|s|x|true|verdadero|1|yes|ya)$/.test(n)) return 'Sí';
        if (/^(no|n|false|falso|0|ninguno|nunca)$/.test(n)) return 'No';
        return txt(v);        // si dice otra cosa, se respeta tal cual
      };
      marketing = mk.rows.map(r => ({
        ...
        show: mShow ? siNo(r[mShow]) : 'Sin especificar',
        hap:  mHap ? siNo(r[mHap]) : 'Sin especificar',
        cli:  mCli ? txt(r[mCli]) : ''
    ...
    try {
      ...
      // ===== Cobros reales desde la pestaña INGRESOS, ligados por folio =====
      const cobros = {};          // folio -> { monto, n }
      let ingresosOk = false, ingresosMotivo = '';
      try {
        const ing = await leer('fin_ingresos');
        if (!ing.headers.length) throw new Error('No se pudo leer la pestaña INGRESOS.');
        const Hi = ing.headers;
        const iDesc = col(Hi, 'Descripción', 'Descripcion', 'Concepto', 'Referencia', 'No. de Referencia');
        const iTot  = colMonto(Hi, ing.rows, ['Total', 'Monto', 'Importe', 'Pago']);
        if (!iDesc || !iTot) throw new Error('INGRESOS necesita una columna de descripción/folio y una de total.');
        ing.rows.forEach(r => {
          const d = txt(r[iDesc]);
          if (!d) return;
          // El folio puede venir solo o dentro de un texto ("Anticipo E12-26")
          const m = d.toUpperCase().match(/\b(COT-)?[A-Z]{1,2}\d+-\d{2}\b/);
          const f = m ? m[0] : d.toUpperCase();
          const monto = num(r[iTot]);
          if (monto === null) return;
          if (!cobros[f]) cobros[f] = { monto: 0, n: 0 };
          cobros[f].monto += monto;
          cobros[f].n++;
        });
        ingresosOk = true;
      } catch (e) { ingresosMotivo = String(e && e.message ? e.message : e); }
      ...
      cx.rows.forEach(r => {
        ...
        const pagHoja = cPag ? numCx(r[cPag]) : 0;
        // Si hay cobros registrados en INGRESOS para este folio, ésos mandan
        const cob = cobros[String(ref).toUpperCase()];
        const pag = (ingresosOk && cob) ? cob.monto : pagHoja;
        const pc  = (ingresosOk && cob) ? (tot - cob.monto)
                                        : (cPC ? numCx(r[cPC]) : (tot - pagHoja));
        ...
        rows.push({
          ...
          pagHoja: pagHoja,
          cobros: (cob ? cob.n : 0),
          difiere: (ingresosOk && cob && Math.abs(cob.monto - pagHoja) > 1),
      ...
      cxc = {
        ...
        ingresosOk: ingresosOk,
        ingresosMotivo: ingresosMotivo,
        conDiferencia: rows.filter(x => x.difiere).length,
    ...
    // ===== Embudo nuevo: leads -> cotizaciones -> ventas =====
    let embudo = { ok: false, motivo: '' };
    try {
      const [cot, lds, shw] = await Promise.all([leer('cotizaciones'), leer('leads'), leer('showroom')]);
      const Hq = cot.headers;
      if (!Hq.length) throw new Error('Todavía no hay pestaña de COTIZACIONES con datos.');
      const qRef = col(Hq, 'No. de Referencia', 'Folio');
      const qSt  = col(Hq, 'Status');
      const qF   = col(Hq, 'Fecha del Cierre', 'Fecha');
      const qCli = col(Hq, 'Cliente');
      const qTipo = col(Hq, 'TIPO CLIENTE', 'Tipo Cliente');
      const qVia = col(Hq, 'Forma de Contacto');
      const qSR  = col(Hq, 'Visito Showroom', 'Visitó Showroom');
      const qCant = col(Hq, 'Cantidad'), qPU = col(Hq, 'Precio Unitario'), qEnv = col(Hq, 'Envio', 'Envío');

      // Una cotización = un folio, aunque tenga varios productos
      const porFolio = {};
      cot.rows.forEach(r => {
        const f = qRef ? txt(r[qRef]) : '';
        if (!f) return;
        if (!porFolio[f]) porFolio[f] = { total: 0, st: '', d: null, cli: '', tipo: '', via: '', sr: '' };
        const q = porFolio[f];
        q.total += (num(r[qCant]) || 0) * (num(r[qPU]) || 0) + (qEnv ? (num(r[qEnv]) || 0) : 0);
        if (!q.st && qSt) q.st = txt(r[qSt]);
        if (q.d === null && qF) q.d = fechaNum(r[qF]);
        if (!q.cli && qCli) q.cli = txt(r[qCli]);
        if (!q.tipo && qTipo) q.tipo = txt(r[qTipo]);
        if (!q.via && qVia) q.via = txt(r[qVia]);
        if (!q.sr && qSR) q.sr = txt(r[qSR]);
      });

      const lF = lds.headers.length ? col(lds.headers, 'Fecha') : null;
      const lVia = lds.headers.length ? col(lds.headers, 'Cómo llegó', 'Como llego') : null;
      const sF = shw.headers.length ? col(shw.headers, 'Fecha') : null;

      embudo = {
        ok: true, motivo: '',
        cotizaciones: Object.keys(porFolio).map(f => ({
          folio: f, d: porFolio[f].d, cli: porFolio[f].cli, total: porFolio[f].total,
          st: porFolio[f].st || 'Pendiente',
          cat: /vendida/i.test(porFolio[f].st) ? 'vendida'
             : (/rechazad/i.test(porFolio[f].st) ? 'rechazada' : 'abierta'),
          tipo: porFolio[f].tipo || 'Sin especificar',
          via: porFolio[f].via || 'Sin especificar',
          sr: siNoGlobal(porFolio[f].sr)
        })),
        leads: lds.rows.map(r => ({
          d: lF ? fechaNum(r[lF]) : null,
          via: lVia ? (txt(r[lVia]) || 'Sin especificar') : 'Sin especificar'
        })).filter(x => x.d !== null || lF === null),
        showroom: shw.rows.map(r => ({ d: sF ? fechaNum(r[sF]) : null })).filter(x => x.d !== null || sF === null)
      };
    } catch (e) {
      embudo = { ok: false, motivo: String(e && e.message ? e.message : e),
                 cotizaciones: [], leads: [], showroom: [] };
    }
    ...
    // ===== Flujo del año: qué entró y qué salió, por concepto =====
    // Los traspasos entre cuentas propias no son ingreso ni gasto: solo mueven dinero.
    let flujo = null;
    try {
      // No son flujo real: mueven dinero entre cuentas propias.
      // Pagar la tarjeta no es un gasto (el gasto fue cuando se usó la tarjeta).
      const NO_ES_FLUJO = /traspaso|transferencia entre cuentas|entre cuentas|movimiento interno|pago de tarjeta|pago a tarjeta|pago tarjeta|pago de tarjetas|tarjeta de credito|tarjeta de credito/i;
      const anioF2 = new Date().getFullYear();

      // INGRESOS y EGRESOS viven SOLO en el archivo de finanzas. Leer también
      // Operación 2026 duplicaba cada movimiento y disparaba los totales.
      const [a1, a2] = await Promise.all([leer('fin_ingresos'), leer('fin_egresos')]);
      const partesIng = a1.headers.length ? [a1] : [];
      const partesEgr = a2.headers.length ? [a2] : [];

      const resumir = (partes) => {
        const porConcepto = {}, porMes = {}, vistos = {};
        let total = 0, traspasos = 0, nTraspasos = 0, sinFecha = 0, repetidos = 0, montoRepetido = 0;
        partes.forEach(P => {
          const Hf2 = P.headers;
          const cCon = col(Hf2, 'Concepto');
          const cCat = col(Hf2, 'Categoría', 'Categoria');
          const cTot2 = col(Hf2, 'Total', 'Monto', 'Importe');
          const cFec2 = col(Hf2, 'Fecha');
          const cDes2 = col(Hf2, 'Descripción', 'Descripcion');
          if (!cTot2) return;
          P.rows.forEach(r => {
            const m = num(r[cTot2]);
            if (m === null || !m) return;
            const d = cFec2 ? fechaNum(r[cFec2]) : null;
            if (d === null) { sinFecha++; return; }
            if (Math.floor(d / 10000) !== anioF2) return;
            const etiqueta = txt(cCon ? r[cCon] : '') || txt(cCat ? r[cCat] : '') || 'Sin concepto';
            const texto = norm(etiqueta + ' ' + txt(cDes2 ? r[cDes2] : '') + ' ' +
                               txt(cCat ? r[cCat] : ''));
            if (NO_ES_FLUJO.test(texto)) { traspasos += m; nTraspasos++; return; }
            // Mismo día, mismo monto y mismo texto: es el mismo movimiento capturado dos veces
            const huella = d + '|' + m.toFixed(2) + '|' + norm(texto).slice(0, 60);
            if (vistos[huella]) { repetidos++; montoRepetido += m; return; }
            vistos[huella] = true;
            total += m;
            if (!porConcepto[etiqueta]) porConcepto[etiqueta] = { concepto: etiqueta, monto: 0, n: 0 };
            porConcepto[etiqueta].monto += m;
            porConcepto[etiqueta].n++;
            const mm = Math.floor(d / 100) % 100;
            porMes[mm] = (porMes[mm] || 0) + m;
          });
        });
        const lista = Object.keys(porConcepto).map(k => porConcepto[k])
          .map(x => ({ concepto: x.concepto, monto: Math.round(x.monto * 100) / 100, n: x.n }))
          .sort((a, b) => b.monto - a.monto);
        return {
          total: Math.round(total * 100) / 100,
          traspasos: Math.round(traspasos * 100) / 100, nTraspasos,
          repetidos, montoRepetido: Math.round(montoRepetido * 100) / 100,
          conceptos: lista,
          porMes: Array.from({ length: 12 }, (_, i) => ({
            mes: i + 1, monto: Math.round((porMes[i + 1] || 0) * 100) / 100
          }))
        };
      };

      const ingresos = resumir(partesIng);
      const egresos = resumir(partesEgr);
      flujo = {
        anio: anioF2,
        ingresos, egresos,
        resultado: Math.round((ingresos.total - egresos.total) * 100) / 100,
        porMes: Array.from({ length: 12 }, (_, i) => ({
          mes: i + 1,
          ingresos: ingresos.porMes[i].monto,
          egresos: egresos.porMes[i].monto,
          resultado: Math.round((ingresos.porMes[i].monto - egresos.porMes[i].monto) * 100) / 100
        })).filter(x => x.ingresos || x.egresos)
      };
    } catch (e) {
      flujo = { error: String(e && e.message ? e.message : e) };
    }

    return res.status(200).json({ ventas: out, marketing: marketing, metas: metas, cxc: cxc, funnel: funnel, embudo: embudo, flujo: flujo });
