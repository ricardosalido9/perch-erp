...
module.exports = async (req, res) => {
  try {
    ...
    try {
      ...
      const ID_OPERACION = '1cbRHK4_-WxCd8q7hemw-PYrt3opZ5GSn5tzghGpByV4';

      const juntar = async (key, hojaAlterna) => {
        const partes = [];
        const a = await leer(key);
        if (a.headers.length) partes.push(a);
        try {
          const b = await leerDe(ID_OPERACION, hojaAlterna);
          if (b.headers.length) partes.push(b);
        } catch (e) { /* no siempre existe */ }
        return partes;
      };
      const [partesIng, partesEgr] = await Promise.all([
        juntar('fin_ingresos', 'INGRESOS'), juntar('fin_egresos', 'EGRESOS')
      ]);

      const resumir = (partes) => {
        const porConcepto = {}, porMes = {};
        let total = 0, traspasos = 0, nTraspasos = 0, sinFecha = 0;
        partes.forEach(P => {
          const Hf2 = P.headers;
