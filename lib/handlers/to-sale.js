...
module.exports = async (req, res) => {
  try {
    ...
    // Cotizaciones y Ventas no siempre nombran igual sus columnas. Se copiaba con
    // el nombre de origen, así que lo que en Cotizaciones se llama "Descuento" y
    // en Ventas "% descuento" se escribía en una columna que no existe y se
    // perdía: la venta salía sin el descuento que la cotización sí tenía.
    //
    // Aquí se traduce cada columna a como se llama en Ventas antes de escribir.
    const cfgV = core.areaCfg ? await core.areaCfg('ventas_registro') : core.SHEETS.ventas_registro;
    let headersVenta = [];
    try {
      const vv = await core.readRange(cfgV.id, await core.resolveSheetName(cfgV));
      headersVenta = (vv[0] || []).map(h => String(h).trim()).filter(Boolean);
    } catch (e) { headersVenta = []; }

    // Los nombres que significan lo mismo en las dos hojas
    const ALIAS = [
      ['% descuento', '% Descuento', 'Descuento %', 'Descuento', 'Porcentaje de descuento'],
      ['Descuentos', 'Descuento', 'Monto descuento', 'Importe descuento'],
      ['Productos', 'Producto', 'Item'],
      ['Precio Unitario', 'Precio', 'Precio de Venta'],
      ['Material Extra', 'Material extra', 'Material 2'],
      ['Especificaciones', 'Especificacion', 'Detalles'],
      ['Envio', 'Envío', 'Costo de envío', 'Costo de envio']
    ];
    const comoSeLlamaEnVentas = (k) => {
      const exacto = headersVenta.filter(h => norm(h) === norm(k))[0];
      if (exacto) return exacto;
      const grupo = ALIAS.filter(g => g.some(x => norm(x) === norm(k)))[0];
      if (grupo) {
        for (const cand of grupo) {
          const h = headersVenta.filter(x => norm(x) === norm(cand))[0];
          if (h) return h;
        }
      }
      return null;      // esa columna no existe en Ventas: no se escribe
    };
    const perdidas = {};
    ...
    const registros = filas.map(f => {
      ...
      Object.keys(f).forEach(k => {
        ...
        if (!headersVenta.length) { rec[k] = f[k]; return; }        // sin encabezados, como antes
        const destino = comoSeLlamaEnVentas(k);
        if (destino) rec[destino] = f[k];
        else if (String(f[k] || '').trim() !== '') perdidas[k] = 1;
      });
    ...
    const columnasPerdidas = Object.keys(perdidas);
    ...
    return res.status(200).json({
      ...
      cotizacion: folio,
      marcada: marcada,
      // Columnas de la cotización que no existen en Ventas: su dato se queda
      // atrás y conviene saberlo en vez de descubrirlo revisando la venta.
