...
module.exports = async (req, res) => {
  ...
  await armar({ _body: {
    ...
    desde: +q.desde || 1,
    // Sin rango, se evalúa de enero al mes en curso: los meses que no han pasado
    // entran en cero y ensucian los promedios y el punto de equilibrio.
    hasta: +q.hasta || ((+q.anio && +q.anio < new Date().getFullYear())
  ...
  // El estado completo. El análisis de arriba dice qué mirar; esto es el estado.
  // Va en miles porque con doce meses más totales no cabe al peso, y mezclar
  // decimales en unas columnas y no en otras se lee peor que redondear todo.
  const MES3 = T.MES3;
  const cols = ['CONCEPTO'];
  for (let m = d.desde; m <= d.hasta; m++) cols.push(MES3[m - 1].toUpperCase());
  cols.push('TOTAL', String(d.anio - 1), 'VAR');
  bloques.push({
    tipo: 'tabla', titulo: d.titulo, formato: 'miles',
    nota: 'cifras en miles de pesos · ' + periodo,
    columnas: cols,
    filas: (d.filas || []).filter(f => !f.vacio).map(f => {
      const celdas = [];
      for (let m = d.desde; m <= d.hasta; m++) {
        const v = f.meses[m - 1];
        celdas.push(f.tipo === 'margen'
          ? (v === null || v === undefined ? '' : T.pct(v, 0)) : (v || 0));
      }
      if (f.tipo === 'margen') {
        celdas.push(T.pct(f.total, 0), T.pct(f.anterior, 0),
          (f.total !== null && f.anterior !== null)
            ? ((f.total - f.anterior) * 100).toFixed(0) + ' pp' : '');
      } else {
        celdas.push(f.total || 0, f.anterior || 0);
        const cam = f.anterior ? (f.total - f.anterior) / Math.abs(f.anterior) * 100 : null;
        celdas.push(cam === null ? '' : (cam >= 0 ? '+' : '') + cam.toFixed(0) + '%');
      }
      return {
        concepto: f.concepto,
        nivel: f.nivel,
        fuerte: f.tipo === 'subtotal',
        celdas
      };
    }),
    pie: 'Los renglones calculados no se capturan: salen de la estructura. La columna del ' +
         'año anterior toma los mismos meses.'
  });
  ...
  let notas = {};
  try {
    notas = await notasDe('estado', d.anio, 0);
  } catch (e) { notas = {}; }
