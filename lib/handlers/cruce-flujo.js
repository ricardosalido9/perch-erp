...
module.exports = async (req, res) => {
  try {
    ...
    const agrupadoPor = {}, negativos = {}, fuente = {};
    const mesBanco = { entrada: new Array(13).fill(0), salida: new Array(13).fill(0) };
    ...
    const juntar = (hoja, l) => {
      ...
      // De dónde salió el número, para poder compararlo contra lo que ves en la
      // pestaña: qué columna se sumó, cuántos renglones entraron y cuántos se
      // quedaron fuera por año o por mes. Casi siempre la diferencia con la hoja
      // está en lo que se quedó fuera, no en lo que entró.
      const fu = fuente[l] = { hoja: l === 'entrada' ? 'INGRESOS' : 'EGRESOS',
        columnaMonto: cT || '(no se encontró)', columnaGrupo: cGrupo || '(ninguna)',
        renglones: hoja.rows.length, entraron: 0, otroAnio: 0, fueraDeRango: 0, sinMonto: 0 };
      let suma = 0;
      hoja.rows.forEach(r => {
        const d = fechaNum(r[cF]);
        if (d === null) { sinFecha++; return; }
        if (Math.floor(d / 10000) !== anio) { fu.otroAnio++; return; }
        const m = Math.floor(d / 100) % 100;
        if (m < desde || m > hasta) { fu.fueraDeRango++; return; }
      ...
      if (suma < 0) {
        ...
        for (let m = 1; m <= 12; m++) mesBanco[l][m] = -mesBanco[l][m];
        neg.volteado = true;
      }
      fu.suma = red(suma);
      return suma;
    ...
    return res.status(200).json({
      ...
      hayMapeo: conAlias > 0,
      fuente: Object.keys(fuente).map(l => fuente[l]),
      // Mes a mes. Si la diferencia se concentra en uno o dos meses es captura;
      // si está repartida en todos, es que falta mapear algo.
      porMes: (() => {
        const out = [];
        for (let m = desde; m <= hasta; m++) {
          const fe = fEnt ? (fEnt.meses[m - 1] || 0) : 0;
          const fs2 = fSal ? Math.abs(fSal.meses[m - 1] || 0) : 0;
          const be = mesBanco.entrada[m], bs = mesBanco.salida[m];
          if (!fe && !fs2 && !be && !bs) continue;
          out.push({ mes: m,
            flujoEntradas: red(fe), ingresos: red(be), difEntradas: red(fe - be),
            flujoSalidas: red(fs2), egresos: red(Math.abs(bs)), difSalidas: red(fs2 - Math.abs(bs)) });
        }
        return out;
      })(),
