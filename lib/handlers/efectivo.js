...
// Las pestañas de efectivo empiezan en la columna D (índice 3)
const COL_INICIO = 3;
...
async function leerCaja(hoja) {
  ...
  // Los encabezados están en la primera fila que tenga "Fecha" en la columna D
  let hr = 0;
  for (let i = 0; i < Math.min(12, values.length); i++) {
    const f = values[i] || [];
    if (norm(f[COL_INICIO]) === 'fecha') { hr = i; break; }
  }
  const cab = values[hr] || [];
  const headers = [];
  ...
  return { headers, rows, filaEncabezados: hr + 1 };
}
  ...
  try {
    ...
    if (body.guardar) {
      ...
      for (let i = hoja.rows.length - 1; i >= 0; i--) {
        const v = iSaldo !== -1 ? num(hoja.rows[i][H[iSaldo]]) : 0;
        if (v || iSaldo === -1) { saldoPrev = v; break; }
      }
      ...
      const rango = "'" + caja + "'!" + letra(COL_INICIO) + destino + ':' +
                    letra(COL_INICIO + H.length - 1) + destino;
    ...
    const CAT_ARCHIVO = '1cKns85GvnfM3Himu_a9AdwFEenP0ofTu8ou4jyiLNoI';
    ...
    const armar = (hoja, nombre) => {
      ...
      let entradas = 0, salidas = 0, saldo = 0;
      const movimientos = [];
      const porConcepto = {};
      hoja.rows.forEach(r => {
