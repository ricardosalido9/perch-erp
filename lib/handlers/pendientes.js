...
module.exports = async (req, res) => {
  try {
    ...
    // Primero las que coinciden en producto Y material.
    const exacto = filtro.length
      ? pend.filter(x => filtro.some(f =>
          f.p === norm(x.producto) && (f.m === null || f.m === norm(x.material))))
      : pend;

    // Y si de algún producto no salió ninguna, se buscan por producto solo.
    //
    // Antes, si el material del pedido estaba escrito distinto al de la venta
    // —"Encino Negro" contra "encino", o un espacio de más— la venta simplemente
    // no aparecía y la ventana decía que nadie había pedido ese producto. El
    // pedido existía y la venta también; lo que no coincidía era una palabra.
    // Ahora salen igual, marcadas, y quien reparte decide.
    const salida = exacto.slice();
    if (filtro.length) {
      const yaHay = {};
      exacto.forEach(x => { yaHay[norm(x.producto)] = 1; });
      filtro.forEach(f => {
        if (yaHay[f.p] || f.m === null) return;
        pend.forEach(x => {
          if (norm(x.producto) !== f.p) return;
          if (salida.indexOf(x) !== -1) return;
          salida.push(Object.assign({}, x, {
            materialDistinto: true,
            materialDelPedido: (filtro.filter(z => z.p === f.p)[0] || {}).m || ''
          }));
        });
      });
    }
