...
module.exports = async (req, res) => {
  try {
    ...
    try {
      ...
      out.detalle = msg;
      // Drive contesta "File not found" en DOS casos que se ven igual desde aquí:
      // que la carpeta no exista, y que exista pero no esté compartida con la
      // cuenta de servicio. No se pueden distinguir sin abrirla a mano, así que
      // se dice cómo distinguirlas en vez de adivinar.
      if (/not found|no se encontró|404/i.test(msg)) {
        out.ligaParaRevisar = 'https://drive.google.com/drive/folders/' + carpeta;
        out.comoSaberCual = [
          'Abre la liga de arriba con tu cuenta de Perch.',
          'Si NO abre: el id está mal o la carpeta se borró. Entra a la carpeta correcta ' +
            'en Drive y copia el id de la barra de direcciones, lo que va después de /folders/.',
          'Si SÍ abre: la carpeta existe pero no está compartida. Compártela con ' +
            cuenta + ' como Administrador de contenido.',
          'Y en los dos casos: la carpeta tiene que vivir en una Unidad Compartida, ' +
            'no en Mi unidad.'
        ];
      }
      return res.status(200).json(out);
