...
// El mismo cruce sirve para los dos lados: las facturas que emitimos y les falta
// su complemento, y las que nos emitieron y nos falta que nos manden el suyo.
// Es literalmente la misma cuenta con otras dos pestañas, así que se parametriza
// en vez de copiarse: dos copias de la misma lógica se separan solas, y esto ya
// nos pasó con los nombres de las columnas.
const LADOS = {
  emitidos: {
    area: 'cfdi_vigentes',
    pestanaAlt: ['EMITIDOS', 'Vigentes', 'VIGENTES'],
    comp: ['COMPLEMENTOS EMITIDOS', 'Complementos emitidos', 'COMPLEMENTOS DE PAGO',
           'Complementos de Pago', 'Complementos de pago', 'Complementos', 'Pagos',
           'COMPLEMENTOS', 'Complemento'],
    // La contraparte de una factura emitida es el cliente
    quien: ['Nombre Receptor', 'Receptor', 'Cliente'],
    rfc: ['RFC Receptor'],
    cruzaCon: 'fin_cxc',
    titulo: 'Facturas emitidas sin su complemento',
    // A quién hay que perseguir
    persigue: 'Nosotros tenemos que emitir el complemento.'
  },
  recibidos: {
    area: 'cfdi_prov_vigentes',
    pestanaAlt: ['RECIBIDOS', 'Recibidos', 'RECIBIDAS'],
    comp: ['COMPLEMENTOS RECIBIDOS', 'Complementos recibidos', 'COMPLEMENTOS DE PAGO RECIBIDOS',
           'Complementos recibidas', 'COMPLEMENTOS RECIBIDAS'],
    quien: ['Nombre Emisor', 'Emisor', 'Proveedor'],
    rfc: ['RFC Emisor'],
    cruzaCon: null,
    titulo: 'Facturas recibidas sin su complemento',
    persigue: 'El proveedor tiene que mandarnos el complemento.'
  }
};
...
module.exports = async (req, res) => {
  try {
    ...
    const L = LADOS[String(body.lado || 'emitidos').toLowerCase()] || LADOS.emitidos;
    // 0 = todos los años. Por omisión, el año en curso: con tres años de CFDIs
    // encima, verlos todos juntos no deja distinguir lo que hay que perseguir hoy.
    const anioFiltro = (body.anio === 0 || body.anio === '0') ? 0
                       : (+body.anio || new Date().getFullYear());
    let cfgC = core.areaCfg ? await core.areaCfg(L.area) : core.SHEETS[L.area];
    // El área de recibidos puede no estar declarada: se cae al mismo archivo de
    // CFDIs, que es donde viven las dos pestañas.
    if (!cfgC || !cfgC.id) {
      cfgC = core.areaCfg ? await core.areaCfg('cfdi_vigentes') : core.SHEETS.cfdi_vigentes;
      if (cfgC && cfgC.id) cfgC = Object.assign({}, cfgC, { sheetName: L.pestanaAlt[0] });
    }
    if (!cfgC || !cfgC.id) {
    ...
    let [vig, comp, cxc] = await Promise.all([
      leerHoja(cfgC.id, cfgC.sheetName || L.pestanaAlt[0]),
      leerHoja(cfgC.id, L.comp[0]),
      L.cruzaCon ? leerArea(L.cruzaCon) : Promise.resolve({ headers: [], rows: [] })
    ]);
    // Si la pestaña de facturas del lado no existe con ese nombre, se prueban los otros
    if (!vig.headers.length) {
      for (const n of L.pestanaAlt) {
        vig = await leerHoja(cfgC.id, n);
        if (vig.headers.length) break;
      }
    }
    // La pestaña de complementos puede llamarse distinto en cada archivo. Faltaba
    // "COMPLEMENTOS EMITIDOS", que es como se llama en la descarga nueva del SAT:
    // al no encontrarla, ningún complemento ligaba y las 232 facturas salían como
    // pendientes. Se prueban los nombres conocidos y, si ninguno existe, se busca
    // entre las pestañas reales del archivo en vez de rendirse.
    if (!comp.headers.length) {
      for (const nombre of L.comp) {
    ...
    if (!comp.headers.length) {
      let todas = [];
      try { todas = await core.listTabs(cfgC.id); } catch (e) { todas = []; }
      // La que hable de complementos y no sea de recibidos
      const cand = (L.area === 'cfdi_prov_vigentes')
        ? todas.filter(t => /complement/i.test(t) && /recib/i.test(t))[0]
        : (todas.filter(t => /complement/i.test(t) && !/recib/i.test(t))[0] ||
           todas.filter(t => /complement/i.test(t))[0]);
      if (cand) { comp = await leerHoja(cfgC.id, cand); pestanaComp = cand; }
    }
    if (!vig.headers.length) {
      let pest = [];
    ...
    const vRecep = col(V, ...L.quien);
    const vRfc = col(V, ...L.rfc);
    ...
    const porFactura = {};
    // Cuando no hay columna de relación, el respaldo: por proveedor y monto
    const porProvMonto = {};
    let compsSinRelacion = 0;
    let columnaRelacion = '';
    if (comp.headers.length) {
      const cRel = col(comp.headers, 'UUID CFDI Pago', 'UUIDs Facturas relacionadas', 'UUID Documento Relacionado', 'IdDocumento', 'UUIDRel', 'UUID Relacion', 'UUID Relación');
      columnaRelacion = cRel || '';
      ...
      // La descarga de COMPLEMENTOS RECIBIDOS no trae la columna que dice a qué
      // factura paga cada complemento: solo trae UUID, emisor, monto y fecha. Sin
      // esa columna no se puede ligar por UUID, y todas las facturas saldrían
      // pendientes aunque estén pagadas.
      //
      // En ese caso se liga por proveedor y monto, que es lo único que hay. NO es
      // lo mismo: dos facturas del mismo proveedor por el mismo importe se
      // confunden entre sí. Por eso cada renglón ligado así queda marcado, para
      // que quien lo lea sepa que es una coincidencia y no un dato.
      if (!cRel) {
        const cRfcEm = col(comp.headers, 'RFC Emisor');
        const cMonto2 = col(comp.headers, 'Monto', 'Total');
        if (cRfcEm && cMonto2) comp.rows.forEach(r => {
          if (/cancelad/i.test(txt(cEst ? r[cEst] : ''))) return;
          const k = norm(r[cRfcEm]) + '|' + Math.round(num(r[cMonto2]));
          (porProvMonto[k] = porProvMonto[k] || []).push({
            uuid: txt(cUUID ? r[cUUID] : ''), fecha: txt(cFec ? r[cFec] : ''),
            dia: cFec ? fechaNum(r[cFec]) : null,
            monto: num(r[cMonto2]), porMonto: true
          });
        });
      }
    ...
    // Qué años hay, para poder ofrecerlos en la pantalla
    const aniosConDatos = {};
    if (vFecha) vig.rows.forEach(r => {
      const d2 = fechaNum(r[vFecha]);
      if (d2) aniosConDatos[Math.floor(d2 / 10000)] = 1;
    });
    ...
    vig.rows.forEach(r => {
      ...
      if (!u) return;
      if (anioFiltro) {
        const d2 = vFecha ? fechaNum(r[vFecha]) : null;
        if (d2 && Math.floor(d2 / 10000) !== anioFiltro) { deOtroAnio++; return; }
      }
      ...
      // Es PPD: se revisa si tiene complemento. Si la hoja de complementos no
      // trae la columna de relación, se cae al respaldo por proveedor y monto.
      let cs = porFactura[u] || [];
      let ligadoPorMonto = false;
      if (!cs.length && !columnaRelacion) {
        const k = norm(base.rfc) + '|' + Math.round(base.total || 0);
        if ((porProvMonto[k] || []).length) { cs = porProvMonto[k]; ligadoPorMonto = true; }
      }
    ...
    return res.status(200).json({
      ok: true,
      lado: String(body.lado || 'emitidos').toLowerCase(),
      anio: anioFiltro,
      anios: Object.keys(aniosConDatos).map(Number).sort((a, b) => b - a),
      deOtroAnio,
      titulo: L.titulo,
      persigue: L.persigue,
      // De dónde salió cada cosa. Cuando "faltan todos los complementos" casi
      // siempre es que no se encontró la pestaña o la columna, no que falten.
      lectura: {
        pestanaComplementos: pestanaComp || (comp.headers.length ? 'encontrada' : '(NO SE ENCONTRÓ)'),
        complementosLeidos: comp.rows.length,
        columnaRelacion: columnaRelacion || '(NO SE ENCONTRÓ)',
        // Sin columna de relación se liga por proveedor y monto, que no es lo
        // mismo: dos facturas iguales del mismo proveedor se confunden.
        ligadoPorMonto: !columnaRelacion && Object.keys(porProvMonto).length > 0,
        complementosSinRelacion: compsSinRelacion,
        encabezadosComplementos: comp.headers.filter(Boolean)
      },
      faltaComplemento, conComplemento, pue, canceladas, sinEnCxc,
      // Las mismas facturas, juntadas por cliente: es como se persigue el complemento,
      // porque se le llama al cliente, no a la factura.
      faltaComplementoPorCliente: (() => {
        const g = {};
        faltaComplemento.forEach(x => {
          const k = x.receptor || '(sin receptor)';
          if (!g[k]) g[k] = { cliente: k, rfc: x.rfc, n: 0, total: 0, falta: 0,
                              urgentes: 0, facturas: [] };
          g[k].n++;
          g[k].total += x.total || 0;
          g[k].falta += (x.falta != null ? x.falta : x.total) || 0;
          if (x.urgente) g[k].urgentes++;
          g[k].facturas.push(x);
        });
        return Object.keys(g).map(k => {
          const c = g[k];
          c.total = Math.round(c.total * 100) / 100;
          c.falta = Math.round(c.falta * 100) / 100;
          c.facturas.sort((a, b) => (a.dia || 0) - (b.dia || 0));
          return c;
        }).sort((a, b) => (b.urgentes - a.urgentes) || (b.falta - a.falta));
      })(),
      diagnostico: {
