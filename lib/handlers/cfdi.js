...
module.exports = async (req, res) => {
  try {
    ...
    const [vig, comp, cxc] = await Promise.all([
      leerHoja(cfgC.id, cfgC.sheetName || 'Vigentes'),
      leerHoja(cfgC.id, 'Complementos'),
      leerArea('fin_cxc')
    ]);
    if (!vig.headers.length) {
      return res.status(400).json({ error: 'No se pudo leer la pestaña de facturas vigentes.' });
    }
    ...
    vig.rows.forEach(r => {
      ...
      // Solo las facturas de ingreso: los complementos y egresos se ven aparte
