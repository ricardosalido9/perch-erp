...
module.exports = async (req, res) => {
  try {
    ...
    if (!comp.headers.length) {
      for (const nombre of ['COMPLEMENTOS EMITIDOS', 'Complementos emitidos',
                            'COMPLEMENTOS DE PAGO', 'Complementos de Pago',
                            'Complementos de pago', 'Pagos', 'COMPLEMENTOS', 'Complemento']) {
    ...
    if (!comp.headers.length) {
      ...
      const cand = todas.filter(t => /complement/i.test(t) && !/recib/i.test(t))[0] ||
