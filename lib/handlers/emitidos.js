...
// El UUID a veces viene con espacios, en minúsculas, o con un apóstrofo delante
// si Google lo guardó como texto. Y la columna de relación puede traer VARIOS uuid separados por
// coma cuando un complemento paga más de una factura. Se compara sin adornos.
function claveUUID(v) {
  return String(v == null ? '' : v).toUpperCase().replace(/[^0-9A-F]/g, '');
}
function uuidsDe(v) {
  return String(v == null ? '' : v).split(/[,;\s|]+/)
    .map(claveUUID).filter(x => x.length >= 24);
}
...
// Conceptos de ingreso que por su naturaleza no llevan factura
const NO_LLEVA_FACTURA = /traspaso|reembolso|aportacion|aportación|capital social|inversion|inversión|pago de tarjeta|deuda|garantia|garantía|socios|financieros|saldo a favor/i;
...
// La pestaña con el nombre nuevo, o con el viejo si el archivo no está migrado
async function leerConAlt(id, nombres) {
  for (const n of nombres) {
    if (!n) continue;
    const r = await leerHoja(id, n);
    if (r.headers && r.headers.length) return r;
  }
  return { headers: [], rows: [] };
}
...
module.exports = async (req, res) => {
  try {
    ...
    const [emi, comp, ing] = await Promise.all([
      leerConAlt(CFG.ARCHIVOS.CFDI_EMITIDOS, [CFG.PESTANAS.cfdiEmitidos, 'EMITIDOS', 'VIGENTES']),
      leerConAlt(CFG.ARCHIVOS.CFDI_EMITIDOS, [CFG.PESTANAS.compEmitidos, 'COMPLEMENTOS EMITIDOS', 'COMPLEMENTOS DE PAGO']),
    ...
    // ---- Complementos de pago, ligados por el UUID de la factura pagada ----
    // Esa columna cambió de nombre entre descargas del SAT: hoy es "UUID CFDI
    // Pago" y antes era "UUIDRel". Buscar solo el nombre viejo hacía que NINGÚN
    // complemento ligara y todas las PPD salieran como pendientes.
    const complementos = [];
    let columnaMontoUsada = '';
    if (comp.headers.length) {
      const Hc = comp.headers;
      const kRel = col(Hc, 'UUID CFDI Pago', 'UUIDs Facturas relacionadas', 'UUID Documento Relacionado', 'IdDocumento', 'UUIDRel', 'UUID Relacion', 'UUID Relación', 'UUID Relacionado');
      const kFec = col(Hc, 'Fecha Emision', 'Fecha Emisión', 'Fecha');
      // Cuál columna trae el importe del pago. En la descarga del SAT conviven
      // 'Total', 'Monto' e 'Importe Pagado', y en algunos casos 'Total' viene en
      // cero porque el CFDI de pago no lleva total. Se prueban en orden y se usa
      // la primera que de verdad tenga números.
      const kTot = (() => {
        // "Importe cobrado" es como se llama en la descarga nueva del SAT. Sin
        // ella el complemento se ligaba bien pero valía cero, que se ve igual de
        // mal que no ligarlo.
        const candidatas = ['Importe cobrado', 'Importe Cobrado', 'Importe Pagado',
                            'ImpPagado', 'Monto', 'Monto Pagado',
                            'Total', 'BASE', 'Base'];
        let mejor = null, mejorSuma = 0;
        candidatas.forEach(nombre => {
          const c = col(Hc, nombre);
          if (!c) return;
          const suma = comp.rows.reduce((a, r) => a + Math.abs(num(r[c])), 0);
          if (suma > mejorSuma) { mejorSuma = suma; mejor = c; }
        });
        return mejor;
      })();
      columnaMontoUsada = kTot || '';
      ...
      if (kRel) comp.rows.forEach(r => {
        ...
        complementos.push({
          ...
          rel: claveUUID(r[kRel]),
          relaciona: uuidsDe(r[kRel]),
          fecha: d, fechaTexto: texto(d),
    ...
    if (ing.headers.length) {
      ...
      const iMet = col(Hi, 'Método de cobro', 'Metodo de cobro', 'Método de pago');
      if (iF && iT) ing.rows.forEach((r, i) => {
        ...
        const metodo = txt(iMet ? r[iMet] : '');
        // A este cruce solo entra lo que DEBERÍA tener factura: el cobro de una
        // venta. Quedan fuera los traspasos entre cuentas, los reembolsos, las
        // aportaciones de capital, las inversiones, los pagos de tarjeta y la
        // deuda, porque ninguno de esos genera un CFDI.
        if (NO_LLEVA_FACTURA.test(concepto)) return;
        // Y el efectivo tampoco: no entró al banco, así que cruzarlo contra
        // complementos no dice nada.
        if (/^efectivo$/i.test(metodo)) return;
        ingresos.push({
    ...
    complementos.forEach(c => {
      // Un complemento puede pagar varias facturas: cuenta para todas
      const lista = (c.relaciona && c.relaciona.length) ? c.relaciona : (c.rel ? [c.rel] : []);
      lista.forEach(u => { (porUUID[u] = porUUID[u] || []).push(c); });
    });
    ...
    return res.status(200).json({
      ...
      resumen: {
        ...
        ligadosOtroCliente: ligados.filter(l => !l.mismoCliente && l.papel.cliente).length,
        montoLigado: red(ligados.reduce((a, l) => a + l.ingreso.monto, 0)),
      ...
      ligados: ligados.map(l => ({
        fila: l.ingreso.fila, fecha: l.ingreso.fecha, fechaTexto: l.ingreso.fechaTexto,
        monto: l.ingreso.monto, clienteIngreso: l.ingreso.cliente,
        concepto: l.ingreso.concepto,
        tipoPapel: l.papel.tipo, clientePapel: l.papel.cliente,
        factura: l.papel.factura, fechaPapel: l.papel.fechaTexto,
        mismoCliente: l.mismoCliente
      })).sort((a, b) => b.fecha - a.fecha).slice(0, 300),
      ...
      lectura: {
        ...
        columnaRelacion: comp.headers.length
          ? (col(comp.headers, 'UUID CFDI Pago', 'UUIDs Facturas relacionadas', 'UUID Documento Relacionado', 'IdDocumento', 'UUIDRel', 'UUID Relacion') || '(NO ESTÁ)') : '(sin pestaña)',
        columnaMonto: columnaMontoUsada || '(NO SE ENCONTRÓ)',
