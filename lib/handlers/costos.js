...
module.exports = async (req, res) => {
  try {
    ...
    // proveedor|producto|material -> costo unitario (null si ese proveedor tiene
    // varios precios distintos para el mismo mueble; ahí no se puede adivinar)
    const costoPorProveedor = {};
    const costoProvSinMaterial = {};
    ...
    if (ped.headers.length) {
      ...
      if (pC && pP) ped.rows.forEach(r => {
        ...
        // Mismo criterio pero por proveedor, para los renglones viejos sin pedido
        const pPr = col(ped.headers, 'Proveedor');
        if (pPr && txt(r[pPr])) {
          const kp = norm(r[pPr]) + '|' + norm(pI ? r[pI] : '') + '|' + norm(pM ? r[pM] : '');
          if (costoPorProveedor[kp] === undefined) costoPorProveedor[kp] = cu;
          else if (costoPorProveedor[kp] !== cu) costoPorProveedor[kp] = null;
          const kp2 = norm(r[pPr]) + '|' + norm(pI ? r[pI] : '');
          if (costoProvSinMaterial[kp2] === undefined) costoProvSinMaterial[kp2] = cu;
          else if (costoProvSinMaterial[kp2] !== cu) costoProvSinMaterial[kp2] = null;
        }
        const fol = txt(r[col(ped.headers, 'Folio cliente', 'Folio Cliente', 'Folio')]).toUpperCase();
        if (fol) {
    ...
    if (sF && sI) {
      sal.rows.forEach(r => {
        ...
        // Un cero ESCRITO no es lo mismo que una celda vacía. Cuando la tela la pone
        // el cliente, el renglón cuesta $0 de verdad y ese cero es el dato correcto.
        // Antes se trataba como "sin costo" y se salía a buscar a Pedidos, donde
        // encontraba el mueble hermano y le cargaba su precio: por eso MY29-26 salía
        // al doble. Si la celda trae algo, aunque sea cero, la hoja manda.
        const crudoCosto = sCo ? txt(r[sCo]) : '';
        const crudoCU = sCU ? txt(r[sCU]) : '';
        // Un cero con formato de moneda se ve como "$ -", SIN ningún dígito. Yo estaba
        // buscando un número y por eso lo tomaba como celda vacía. Lo que importa es
        // si la celda trae algo: si trae, la hoja ya dijo cuánto cuesta, aunque sea cero.
        const hayCosto = crudoCosto !== '' || crudoCU !== '';
        let importe = sCo ? num(r[sCo]) : 0;
        let cu = sCU ? num(r[sCU]) : 0;
        let fuente = 'costo de la salida';
        if (!importe && cu) importe = cu * c;
        if (!importe && hayCosto) {
          // Cero explícito: se da por costeado en cero y no se busca en ningún lado
          const d0 = salidas[f] = salidas[f] || { piezas: 0, real: 0, cat: 0, sinCosto: 0,
                                                  prov: {}, aprox: 0, ceros: 0, det: [] };
          d0.piezas += c;
          d0.ceros++;
          const cc0 = costoCat[kp];
          if (cc0 !== undefined) d0.cat += cc0 * c;
          if (sPr && txt(r[sPr])) d0.prov[txt(r[sPr])] = 1;
          if (d0.det.length < 40) {
            d0.det.push({
              producto: txt(r[sI]), material: txt(sM ? r[sM] : ''),
              proveedor: txt(sPr ? r[sPr] : ''), pedido: txt(sP ? r[sP] : ''),
              cantidad: c, costoUnitario: 0, fuente: 'sin costo a propósito',
              motivo: null, sugerencia: null, opciones: [],
              catalogo: (cc0 === undefined) ? null : Math.round(cc0 * 100) / 100
            });
          }
          return;
        }
        ...
        if (!importe && sP) {
          ...
          if (delPedido) { importe = delPedido * c; fuente = 'costo del pedido'; }
            ...
            // Solo se ignora el material cuando la salida NO trae material. Si trae
            // "Tela" y el pedido tiene "Nogal", son piezas distintas: buscar sin
            // material le cargaría a la tela el precio del mueble.
        ...
        if (!importe) {
          ...
          // Igual que arriba: sin material solo si la salida no lo trae
          if (!m.length && !txt(sM ? r[sM] : '')) {
            m = delFolio.filter(x => norm(x.producto) === norm(r[sI]));
          }
        ...
        // Cuarto y último intento: por proveedor + producto + material, sin pedido.
        // Es para los renglones viejos de la hoja, que traen proveedor pero no número
        // de pedido ni costo. Solo se usa si TODOS los pedidos de ese proveedor con
        // ese mueble cuestan lo mismo; si hay varios precios, se deja pendiente en
        // vez de escoger uno. Se marca como aproximado para que se distinga.
        if (!importe && sPr && txt(r[sPr])) {
          const kProv = norm(r[sPr]) + '|' + norm(r[sI]) + '|' + norm(sM ? r[sM] : '');
          const cp = costoPorProveedor[kProv];
          if (cp) {
            importe = cp * c;
            fuente = 'costo del proveedor (aprox)';
          } else if (!txt(sM ? r[sM] : '')) {
            const kProv2 = norm(r[sPr]) + '|' + norm(r[sI]);
            const cp2 = costoProvSinMaterial[kProv2];
            if (cp2) { importe = cp2 * c; fuente = 'costo del proveedor (aprox)'; }
          }
        }
    ...
    if (ped.headers.length) {
      const qF = col(ped.headers, 'Folio cliente', 'Folio Cliente', 'Folio');
    ...
    const folios = {};
    ven.rows.forEach(r => {
      // Una venta cancelada no entra al comparativo: su costo no existe
      if (vSt && /cancelad|anulad/i.test(txt(r[vSt]))) return;
    ...
    const filas = Object.keys(folios).map(f => {
      ...
      // Lo mismo cuando TODOS los renglones traen cero escrito: una venta entera
      // que no le costó nada a la empresa no existe; falta capturar el costo.
      const todoSinCostear = !!(s && s.piezas &&
        ((s.sinCosto && s.real === 0) || (s.real === 0 && s.ceros)));
