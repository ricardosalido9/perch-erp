...
module.exports = async (req, res) => {
  try {
    ...
    if (ped.headers.length) {
      ...
      const pI = col(ped.headers, 'Item'), pM = col(ped.headers, 'Material');
    ...
    const sI = col(sal.headers, 'Item'), sM = col(sal.headers, 'Material');
    ...
    if (sF) sal.rows.forEach(r => {
      ...
      const d = salidas[f] = salidas[f] || { piezas: 0, real: 0, cat: 0, sinCosto: 0, prov: {}, aprox: 0 };
      d.piezas += c;
    ...
    ven.rows.forEach(r => {
      ...
      if (!d.fecha && vFe) d.fecha = txt(r[vFe]);
    });
    ...
    const filas = Object.keys(folios).map(f => {
      ...
      return {
        folio: f, cliente: d.cliente, fecha: d.fecha,
