  ...
  try {
    ...
    if (ped.headers.length) {
      ...
      if (qF && qU) ped.rows.forEach(r => {
        ...
        yaContado[f + '|' + norm(qP ? r[qP] : '') + '|' + kp] = true;
        if (d.det.length < 40) {
          d.det.push({
    ...
    if (sF) salRows.forEach(r => {
      ...
      // Si el pedido ya asignó esta pieza a este folio, no se cuenta dos veces
      if (yaContado[f + '|' + norm(sP ? r[sP] : '') + '|' + kp]) return;
      let cu = exacto[kx];
      let fuente = 'pedido', pedidoUsado = txt(sP ? r[sP] : '');
