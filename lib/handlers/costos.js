  ...
  try {
    ...
    if (sF && sI) {
      sinRepetidos(sal.rows, [sF, sI, sM, sC, sP].filter(Boolean)).forEach(r => {
        ...
        if (!importe) {
          // Misma búsqueda que hace tu fórmula: pedido + producto + material
          const k = norm(sP ? r[sP] : '') + '|' + kp;
          const enc = costoPedido[k];
          if (enc !== undefined) { cu = enc; importe = enc * c; fuente = 'buscado en el pedido ' + txt(sP ? r[sP] : ''); }
        }
        const d = salidas[f] = salidas[f] || { piezas: 0, real: 0, cat: 0, sinCosto: 0, prov: {}, aprox: 0, det: [] };
        d.piezas += c;
        if (!importe) d.sinCosto++;
        else d.real += importe;
        ...
        if (d.det.length < 40) {
          d.det.push({
            ...
            fuente: importe ? fuente : 'sin costo', opciones: [],
