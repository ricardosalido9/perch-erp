...
module.exports = async (req, res) => {
  try {
    ...
    const salidas = {};
    const yaContado = {};      // por folio + producto + material
    const porProducto = {};    // por folio + producto (para detectar dobles conteos)
    if (ped.headers.length) {
      ...
      if (qF && qU) ped.rows.forEach(r => {
        ...
        const kprod = f + '||' + norm(qI ? r[qI] : '');
        porProducto[kprod] = (porProducto[kprod] || 0) + c;
        if (d.det.length < 40) {
          d.det.push({
    ...
    if (sF) salRows.forEach(r => {
      ...
      let c = sC ? num(r[sC]) : 0;
      if (!c) return;
      // 1) Se descuenta lo que el pedido ya cubrió con ese mismo material
      const kprod2 = f + '||' + norm(sI ? r[sI] : '');
      if (yaContado[ky]) {
        const usar = Math.min(yaContado[ky], c);
        yaContado[ky] -= usar;
        porProducto[kprod2] = Math.max(0, (porProducto[kprod2] || 0) - usar);
        c -= usar;
        if (c <= 0) return;
      }
      // 2) Y también lo cubierto por sus COMPONENTES. En Pedidos el mueble se compra
      //    por partes (madera + acero) y en Salidas sale como producto terminado:
      //    si no se descontara, el mismo mueble se contaría dos veces.
      let sospechoso = false;
      if (porProducto[kprod2] > 0) {
        const usar = Math.min(porProducto[kprod2], c);
        porProducto[kprod2] -= usar;
        c -= usar;
        sospechoso = true;
        if (c <= 0) return;
      }
      ...
      if (d.det.length < 30) {
        d.det.push({
          ...
          fuente: (cu === undefined) ? 'sin costo' : (sospechoso ? fuente + ' · REVISAR posible duplicado' : fuente),
