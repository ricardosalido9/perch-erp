...
// Prueba varios nombres de pestaña y se queda con la primera que traiga algo.
// Sirve mientras conviven los nombres viejos y los nuevos.
...
module.exports = async (req, res) => {
  try {
    ...
    const [emi, emiComp, rec, recComp, nom] = await Promise.all([
      // Cada pestaña con su nombre nuevo y, si no existe, con el viejo. Emitidos
      // y recibidos ya no se distinguen por archivo sino por pestaña.
      ...
      leerConAlt(CFG.ARCHIVOS.CFDI_RECIBIDOS, [CFG.PESTANAS.cfdiRecibidos, 'RECIBIDOS', 'VIGENTES']),
      leerConAlt(CFG.ARCHIVOS.CFDI_RECIBIDOS, [CFG.PESTANAS.compRecibidos, 'COMPLEMENTOS RECIBIDOS', 'COMPLEMENTOS DE PAGO']),
      // Los CFDIs de nómina viven en su propio archivo, pestaña NOMINA. Si ahí no
      // están, se busca en el de recibidos, que es donde estaban antes.
      leerConAlt(CFG.ARCHIVOS.NOMINA, [CFG.PESTANAS.nomina, 'NOMINA', 'Nómina', 'Nomina'])
        .then(x => x.headers.length ? x
              : leerConAlt(CFG.ARCHIVOS.CFDI_RECIBIDOS, ['NOMINA', 'Nómina']))
    ]);
    ...
    function bloque(hoja, etiqueta) {
      ...
      // La hoja trae una columna "Mes" y ESA es la que manda, no la fecha de emisión.
      // Una factura timbrada el 7 de agosto puede pertenecer a julio, y así la
      // declara el contador. Antes se filtraba por fecha y por eso el reporte
      // dejaba fuera facturas que sí van en el mes.
      ...
      if (!cFec && !cMes) faltantes.push(etiqueta + ': no tiene columna de Fecha ni de Mes');

      const mesDe = (r) => {
        if (cMes) {
          const m = parseInt(String(r[cMes]).replace(/[^0-9]/g, ''), 10);
          if (m >= 1 && m <= 12) return m;
        }
        const d = cFec ? fechaNum(r[cFec]) : null;
        return d === null ? null : Math.floor(d / 100) % 100;
      };
      const anioDe = (r) => {
        const d = cFec ? fechaNum(r[cFec]) : null;
        return d === null ? anio : Math.floor(d / 10000);
      };
      ...
      const delMes = hoja.rows.filter(r => {
        ...
        return mesDe(r) === mes && anioDe(r) === anio;
      });
      ...
      if (comp && comp.headers.length) {
        // La descarga nueva del SAT nombra esta columna "UUID CFDI Pago": trae el
        // UUID de la factura que se está pagando. Antes se llamaba UUIDRel. Se
        // buscan los dos, porque los archivos viejos siguen usando el nombre viejo.
        const cRel = col(comp.headers, 'UUID CFDI Pago', 'UUIDs Facturas relacionadas',
                         'UUID Documento Relacionado', 'IdDocumento',
                         'UUIDRel', 'UUID Relacion', 'UUID Relación', 'UUID Relacionado');
      ...
      // Los complementos EMITIDOS en el mes, con su base y su IVA. El IVA de un
      // CFDI en parcialidades no se causa al facturar, se causa al cobrar, y el
      // cobro es el complemento. Por eso el IVA trasladado del mes NO es el IVA
      // de todas las facturas: es el de las PUE más el de los complementos.
      let compDelMes = { n: 0, subtotal: 0, iva: 0, total: 0 };
      if (comp && comp.headers.length) {
        const Hc = comp.headers;
        const kFec = col(Hc, 'Fecha Emision', 'Fecha Emisión', 'Fecha');
        const kMes = col(Hc, 'MES', 'Mes');
        const kBase = col(Hc, 'BASE', 'Base', 'Subtotal');
        const kIva = col(Hc, 'IVA', 'IVA 16%');
        const kTot = col(Hc, 'Total', 'Monto');
        const kEst = col(Hc, 'Estado SAT', 'Estado');
        comp.rows.forEach(r => {
          if (kEst && /cancel/i.test(txt(r[kEst]))) return;
          let m = null;
          if (kMes) {
            const x = parseInt(String(r[kMes]).replace(/[^0-9]/g, ''), 10);
            if (x >= 1 && x <= 12) m = x;
          }
          if (m === null && kFec) {
            const d = fechaNum(r[kFec]);
            if (d !== null) m = Math.floor(d / 100) % 100;
          }
          if (m !== mes) return;
          compDelMes.n++;
          compDelMes.subtotal += kBase ? num(r[kBase]) : 0;
          compDelMes.iva += kIva ? num(r[kIva]) : 0;
          compDelMes.total += kTot ? num(r[kTot]) : 0;
        });
        compDelMes.subtotal = red(compDelMes.subtotal);
        compDelMes.iva = red(compDelMes.iva);
        compDelMes.total = red(compDelMes.total);
      }
      ...
      return {
        ...
        totales: {
          ...
          // El que se declara: PUE del mes + complementos cobrados en el mes
          ivaDelPeriodo: red(sum(pue, 'iva') + compDelMes.iva),
          ivaPUE: sum(pue, 'iva'),
          ivaComplementos: compDelMes.iva,
          complementosDelMes: compDelMes,
