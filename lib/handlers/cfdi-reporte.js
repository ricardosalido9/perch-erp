...
module.exports = async (req, res) => {
  try {
    ...
    const [emi, emiComp, rec, recComp, nom] = await Promise.all([
      ...
      leerHoja(CFG.ARCHIVOS.NOMINA, CFG.PESTANAS.nomina)
        .then(x => x.headers.length ? x : leerHoja(CFG.ARCHIVOS.NOMINA, 'NOMINA'))
        .then(x => x.headers.length ? x : leerHoja(CFG.ARCHIVOS.CFDI_RECIBIDOS, 'NOMINA'))
        .then(x => x.headers.length ? x : leerHoja(CFG.ARCHIVOS.CFDI_RECIBIDOS, 'Nómina'))
    ]);
