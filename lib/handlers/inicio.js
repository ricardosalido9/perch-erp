    ...
    try {
      ...
      if (ventas.headers.length && sal2.headers.length) {
        ...
        if (vRef && vProd) ventas.rows.forEach(r => {
          ...
          const kProd = norm(prod) + '|' + norm(mat);
          const hay = enStock[kProd] || 0;
          const donde = (hay > 0) ? deStock : dePedir;
          if (hay > 0) piezasStock += faltan; else piezasPedir += faltan;
          donde[folio] = donde[folio] || { folio: folio, piezas: 0, muebles: [] };
          donde[folio].piezas += faltan;
          donde[folio].muebles.push({
            producto: prod, material: mat, faltan: faltan, enStock: hay,
            proveedores: Object.keys(quienLoHace[kProd] || {}).sort()
          });
        });
