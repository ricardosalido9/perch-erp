    ...
    try {
      ...
      if (ventas.headers.length && sal2.headers.length) {
        ...
        if (vRef && vProd) ventas.rows.forEach(r => {
          ...
          if (!pedidas) return;
          const kSal = norm(folio) + '|' + norm(prod) + '|' + norm(mat);
          const faltan = pedidas - (yaSalio[kSal] || 0);
          if (faltan <= 0) return;                     // ya tiene su salida
          ...
          const componentes = [mat, mat2].filter(x => x && norm(x) !== 'na')
            .map(m => {
              ...
              return {
                ...
                enStock: enStock[k] || 0,
                alcanza: (enStock[k] || 0) >= faltan,
                proveedores: Object.keys(quienLoHace[k] || {}).sort()
              };
            });
          if (!componentes.length) {
            const k = norm(prod) + '|';
            componentes.push({ material: '', enStock: enStock[k] || 0,
              alcanza: (enStock[k] || 0) >= faltan,
              proveedores: Object.keys(quienLoHace[k] || {}).sort() });
          }
        ...
        if (nStock) out.avisos.push({ tipo: 'info', area: 'prov_salidas',
