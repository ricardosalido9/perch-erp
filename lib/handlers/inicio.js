...
module.exports = async (req, res) => {
  try {
    ...
    try {
      const [sal2, stk, cu] = await Promise.all([
        leer('prov_salidas'), leer('op_stock'), leer('costos_unitarios')
      ]);
      if (ventas.headers.length && sal2.headers.length) {
        ...
        // Un mueble puede llevar dos materiales, y hasta ahora el aviso solo
        // miraba el primero: decía que faltaba una pieza cuando en realidad
        // podían faltar dos componentes, o ninguno.
