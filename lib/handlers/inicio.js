  ...
  try {
    ...
    try {
      ...
      if (ped.headers.length) {
        ...
        const pFol = col(Hp, 'Folio cliente', 'Folio');
    ...
    // ===== PRODUCCIÓN: ventas que Operación todavía no atiende =====
    // Una venta se considera atendida cuando ya tiene salida registrada. Aquí se avisa
    // de las que no la tienen, diciendo si el mueble está en stock (se saca) o no
    // (hay que pedirlo), y qué proveedores lo hacen. NO se recomienda proveedor: se
    // listan los que pueden y Nico decide.
    try {
      const [sal2, stk, cu, cat] = await Promise.all([
        leer('prov_salidas'), leer('op_stock'), leer('costos_unitarios'), leer('inventario')
      ]);
      if (ventas.headers.length && sal2.headers.length) {
        const Hv = ventas.headers;
        const vRef = col(Hv, 'No. de Referencia', 'Folio');
        const vProd = col(Hv, 'Productos', 'Producto');
        const vMat = col(Hv, 'Material');
        // El segundo componente NO está en VENTAS: la hoja solo tiene "Material".
        // Vive en el catálogo, donde cada producto trae Material y Material Extra
        // —"Atril Bisbita Grande" es Nogal + Acero—. Por eso el aviso mostraba un
        // solo material aunque el mueble llevara dos.
        // Si VENTAS trae su propia columna de segundo material, esa manda: una
        // venta puede pedir un acabado distinto al que trae el catálogo por
        // omisión. El catálogo es el respaldo, no al revés.
        const vMat2 = col(Hv, 'Material Extra', 'Material extra', 'Material 2', 'Material2',
                          'Segundo Material', 'Material Secundario', 'Acabado', 'Tapizado');
        const segundoMaterial = {};
        if (cat.headers.length) {
          const kP = col(cat.headers, 'Productos', 'Producto');
          const kM = col(cat.headers, 'Material');
          const kM2 = col(cat.headers, 'Material Extra', 'Material extra', 'Material 2',
                          'Material2', 'Segundo Material', 'Acabado', 'Tapizado');
          if (kP && kM2) cat.rows.forEach(r => {
            const extra = txt(r[kM2]);
            if (!extra || /^(na|n\/a|no|-)$/i.test(extra)) return;
            // Por producto + material, y también por producto solo, para cuando
            // el material de la venta no coincide exactamente con el del catálogo
            segundoMaterial[norm(r[kP]) + '|' + norm(kM ? r[kM] : '')] = extra;
            if (!segundoMaterial[norm(r[kP])]) segundoMaterial[norm(r[kP])] = extra;
          });
        }
        const vCant = col(Hv, 'Cantidad');
        const vEnt = col(Hv, 'Status de Entrega', 'Status');
        const Hs2 = sal2.headers;
        const sFol2 = col(Hs2, 'Folio cliente', 'Folio');
        const sProd2 = col(Hs2, 'Productos', 'Producto', 'Item');
        const sMat2 = col(Hs2, 'Material');
        const sCant2 = col(Hs2, 'Cantidad');

        // Lo que ya salió, por folio + mueble + material
        const yaSalio = {};
        if (sFol2 && sProd2) sal2.rows.forEach(r => {
          const k = norm(r[sFol2]) + '|' + norm(r[sProd2]) + '|' + norm(sMat2 ? r[sMat2] : '');
          yaSalio[k] = (yaSalio[k] || 0) + (sCant2 ? num(r[sCant2]) : 0);
        });

        // Piezas disponibles en stock, por mueble + material
        const enStock = {};
        if (stk.headers.length) {
          const kI = col(stk.headers, 'Productos', 'Producto', 'Item');
          const kM = col(stk.headers, 'Material');
          const kD = col(stk.headers, 'Stock inicial', 'Stock', 'Disponible');
          if (kI && kD) stk.rows.forEach(r => {
            const k = norm(r[kI]) + '|' + norm(kM ? r[kM] : '');
            enStock[k] = (enStock[k] || 0) + num(r[kD]);
          });
        }

        // Quién puede fabricar cada mueble, según la lista de costos unitarios
        const quienLoHace = {};
        if (cu.headers.length) {
          const qI = col(cu.headers, 'Productos', 'Producto');
          const qM = col(cu.headers, 'Material 1', 'Material');
          const qP = col(cu.headers, 'Proveedor');
          if (qI && qP) cu.rows.forEach(r => {
            const k = norm(r[qI]) + '|' + norm(qM ? r[qM] : '');
            const p = txt(r[qP]);
            if (!p) return;
            (quienLoHace[k] = quienLoHace[k] || {})[p] = 1;
          });
        }

        const deStock = {}, dePedir = {};
        let piezasStock = 0, piezasPedir = 0;
        if (vRef && vProd) ventas.rows.forEach(r => {
          const folio = txt(r[vRef]);
          const prod = txt(r[vProd]);
          if (!folio || !prod) return;
          // Lo ya entregado no se vuelve a avisar
          if (vEnt && /entregad|cancelad/i.test(txt(r[vEnt]))) return;
          const mat = vMat ? txt(r[vMat]) : '';
          const mat2 = (vMat2 ? txt(r[vMat2]) : '') ||
                       segundoMaterial[norm(prod) + '|' + norm(mat)] ||
                       segundoMaterial[norm(prod)] || '';
          const pedidas = vCant ? (num(r[vCant]) || 0) : 0;
          if (!pedidas) return;
          // Cuánto se le ha surtido a esta venta, POR COMPONENTE. Antes se
          // miraba solo el primer material: al darle salida a ese, el renglón
          // completo desaparecía del aviso aunque el segundo componente siguiera
          // sin salir, y la venta quedaba incompleta sin que nada lo dijera.
          const surtido = (m) => yaSalio[norm(folio) + '|' + norm(prod) + '|' + norm(m)] || 0;

          // El mueble se compone de uno o dos materiales, y para surtirlo de
          // bodega tienen que estar LOS DOS. Antes bastaba con que hubiera del
          // primero, y entonces la venta salía como "surtible" y al ir a darle
          // salida faltaba el otro.
          const mat2DeLaVenta = !!(vMat2 && txt(r[vMat2]));
          const listaMat = [mat, mat2].filter(x => x && norm(x) !== 'na');
          if (!listaMat.length) listaMat.push('');
          const componentes = listaMat.map(m => {
            const k = norm(prod) + '|' + norm(m);
            const faltaEste = pedidas - surtido(m);
            return {
              material: m,
              // El segundo componente puede venir de la venta o deducirse del
              // catálogo. Si se dedujo, es una suposición y hay que poder verlo.
              delCatalogo: (m === mat2 && !mat2DeLaVenta),
              faltan: faltaEste,
              yaSalio: surtido(m),
              enStock: enStock[k] || 0,
              alcanza: (enStock[k] || 0) >= faltaEste,
              proveedores: Object.keys(quienLoHace[k] || {}).sort()
            };
          }).filter(c => c.faltan > 0);       // el que ya salió deja de pedirse

          // Si ya salieron todos los componentes, la venta está surtida
          if (!componentes.length) return;
          // Lo que falta del renglón es lo del componente más atrasado
          const faltan = Math.max.apply(null, componentes.map(c => c.faltan));
          const completo = componentes.every(x => x.alcanza);
          const donde = completo ? deStock : dePedir;
          if (completo) piezasStock += faltan; else piezasPedir += faltan;
          const proveedores = [];
          componentes.forEach(x => x.proveedores.forEach(p => {
            if (proveedores.indexOf(p) === -1) proveedores.push(p);
          }));
          donde[folio] = donde[folio] || { folio: folio, piezas: 0, muebles: [] };
          donde[folio].piezas += faltan;
          donde[folio].muebles.push({
            producto: prod,
            material: componentes.map(x => x.material).filter(Boolean).join(' + '),
            componentes: componentes.map(x => ({
              material: x.material, enStock: x.enStock, alcanza: x.alcanza,
              faltan: x.faltan, yaSalio: x.yaSalio,
              delCatalogo: x.delCatalogo, proveedores: x.proveedores
            })),
            // Cuál es el que frena: es lo que hay que pedir
            falta: componentes.filter(x => !x.alcanza).map(x => x.material).join(' · '),
            faltan: faltan,
            enStock: Math.min.apply(null, componentes.map(x => x.enStock)),
            proveedores: proveedores
          });
        });

        const nStock = Object.keys(deStock).length;
        const nPedir = Object.keys(dePedir).length;
        if (nStock) out.avisos.push({ tipo: 'info', area: 'op_stock',
          titulo: 'Ventas por surtir de stock',
          detalle: nStock + (nStock === 1 ? ' venta necesita ' : ' ventas necesitan ') +
                   piezasStock + ' piezas que ya están en bodega: hay que darles salida',
          n: nStock, detalles: Object.keys(deStock).map(k => deStock[k]).slice(0, 20) });
        if (nPedir) out.avisos.push({ tipo: 'alert', area: 'prov_pedidos',
          titulo: 'Ventas que necesitan pedido al proveedor',
          detalle: nPedir + (nPedir === 1 ? ' venta pide ' : ' ventas piden ') +
                   piezasPedir + ' piezas que no están en stock',
          n: nPedir, detalles: Object.keys(dePedir).map(k => dePedir[k]).slice(0, 20) });
      }
    } catch (e) { /* si falta alguna hoja, el aviso simplemente no sale */ }
