    ...
    const propuestas = movs.map(m => {
      ...
      if (m.tipo === 'INGRESO' && !p.contraparte) {
        ...
        if (cuadra && cuadra.unico) {
          ...
          p.porQue = (p.porQue ? p.porQue + ' ' : '') +
            'El monto le cuadra a ' + cuadra.opciones.length + ' folios: elige cuál.';
        }
      }

      // En un cargo, si el monto cuadra con un pago que Nico solicitó, ya está
      // todo resuelto: proveedor, pedido y concepto salen de ahí.
      if (m.tipo === 'EGRESO' && !p.contraparte) {
        const sol = pagoSolicitado(m.monto);
        if (sol) {
          p.contraparte = sol.proveedor;
          p.pedido = sol.pedido;
          if (!p.concepto || !p.categoria) p.conceptoSugerido = sol.concepto;
          p.confianza = 'probable';
          p.porQue = 'Cuadra con un pago que se pidió a ' + sol.proveedor +
