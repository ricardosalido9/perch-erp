...
module.exports = async (req, res) => {
  try {
    ...
    if (!movs.length) {
      // El lector reconoce los dos formatos de BBVA. Si el PDF trae movimientos
      // pero con otro acomodo —AMEX, Santander, Banorte— no encuentra nada y el
      // mensaje genérico manda a buscar un error que no existe. Aquí se dice.
      const otroBanco = /american express|amex|santander|banorte|banamex|citibanamex|hsbc|scotiabank|inbursa/i;
      const cual = (texto.match(otroBanco) || [])[0];
      if (cual) {
        return res.status(400).json({
          error: 'Este estado de cuenta no es de BBVA.',
          pista: 'El lector reconoce por ahora los dos formatos de BBVA: el estado de ' +
                 'cuenta del mes y el detalle de BBVAnet. El de ' + cual + ' acomoda las ' +
                 'columnas distinto, así que hay que enseñarle ese formato. Manda un PDF ' +
                 'de ejemplo y se agrega.',
          banco: cual
        });
      }
      return res.status(400).json({
