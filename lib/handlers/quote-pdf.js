...
const { cotizacionNueva } = require('../pdf-cot-nueva');
const { aFormatoNuevo } = require('../cot-nueva-datos');
...
module.exports = async (req, res) => {
  try {
    ...
    // Dos formatos: el de siempre y el nuevo que diseñó Pau. Se elige desde la
    // pantalla. Si el nuevo truena por lo que sea, se cae al de siempre en vez de
    // dejar al usuario sin cotización: es un documento que se manda al cliente.
    // El formato de diseño es el predeterminado. El clásico solo se usa si se pide
    // explícitamente, o como respaldo automático cuando el nuevo falla.
    const nuevo = String(body.formato || 'nuevo').toLowerCase() !== 'clasico';
    let pdfBuf, formatoUsado = nuevo ? 'nuevo' : 'clasico', porQueClasico = '';
    if (nuevo) {
      try {
        const d = await aFormatoNuevo(cot, { sinFotos: !!body.sinFotos });
        pdfBuf = await cotizacionNueva(d.cot, {
          tipoDocumento: esVenta ? 'Venta' : 'Cotización',
          piezas: d.piezas, descuentoPct: d.descuentoPct,
          fechaCorta: cot.mesAnio, banco: BANCO,
          // Los términos van en el módulo de términos, en dos columnas fijas
          // como el formato de diseño. Las listas sueltas de antes eran para el
          // formato clásico y ahí se quedan.
          empresa: 'PÉRCH', sitio: 'WWW.PERCH.COM', telefono: '+52 1 56 1801 7975'
        });
      } catch (e) {
        formatoUsado = 'clasico';
        porQueClasico = (e && e.message) || String(e);
      }
    }
    if (!pdfBuf) {
      try {
    ...
    if (body.soloArchivo) {
      ...
      lectura: cot.lectura || null, formato: formatoUsado, porQueClasico: porQueClasico });
    }
