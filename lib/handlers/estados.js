...
module.exports = async (req, res) => {
  try {
    ...
    if (cual === ESTADOS.resultados) {
      ...
      // Cuánto de la venta se va en descuentos, que es la palanca más directa
      const des = buscar('Descuentos y/o devoluciones'), vb = buscar('Ventas Brutas');
      if (des && vb && vb.total) {
        const d1 = des.total / vb.total * 100;
        const d0 = (vb.anterior && des.anterior) ? des.anterior / vb.anterior * 100 : null;
        lectura.push('Los descuentos son el ' + d1.toFixed(1) + '% de la venta bruta' +
          (d0 === null ? '.' : ', contra ' + d0.toFixed(1) + '% del año pasado: ' +
            (d1 < d0 ? 'se está descontando menos.' : 'se está descontando más.')));
      }
      // El gasto que más pesa y el que más se movió
      if (gga) {
        const hijos = filas.filter(f => f.nivel === 2 && f.total &&
          conceptos.filter(c => c.concepto === f.concepto)[0] &&
          /gastos generales|gastos de venta/i.test(
            conceptos.filter(c => c.concepto === f.concepto)[0].padre || ''));
        const mayor = hijos.slice().sort((a, b) => b.total - a.total)[0];
        if (mayor) {
          const fijo = gga.total + (gv ? gv.total : 0);
          lectura.push('El gasto más grande es ' + mayor.concepto + ' con ' +
            dinero(mayor.total) + ', el ' + (mayor.total / fijo * 100).toFixed(0) +
            '% de todo el gasto del periodo.');
        }
        const conAnt = hijos.filter(h => h.anterior && Math.abs(h.anterior) > 1000);
        const salto = conAnt.slice().sort((a, b) =>
          Math.abs(cambio(b.total, b.anterior)) - Math.abs(cambio(a.total, a.anterior)))[0];
        if (salto) {
          const c = cambio(salto.total, salto.anterior);
          lectura.push('El renglón que más se movió contra el año pasado es ' + salto.concepto +
            ': ' + (c >= 0 ? 'subió ' : 'bajó ') + Math.abs(c).toFixed(0) + '%, de ' +
            dinero(salto.anterior) + ' a ' + dinero(salto.total) + '.');
        }
      }
      // El mejor y el peor mes, para saber si el periodo es parejo o depende de uno
      if (vn && nMeses > 2) {
        const conValor = vn.meses.map((v, i) => ({ v: v || 0, i })).filter(x => x.v);
        if (conValor.length > 2) {
          const alto = conValor.slice().sort((a, b) => b.v - a.v)[0];
          const bajo = conValor.slice().sort((a, b) => a.v - b.v)[0];
          lectura.push('El mes más fuerte fue ' + MESN[alto.i] + ' con ' + dinero(alto.v) +
            ' y el más flojo ' + MESN[bajo.i] + ' con ' + dinero(bajo.v) + ': ' +
            (alto.v / bajo.v).toFixed(1) + ' veces de diferencia entre uno y otro.');
        }
      }
      if (uo) {
        const malos = [];
    ...
    // Dos renglones distintos con exactamente el mismo número todos los meses casi
    // siempre significa que los dos se llenaron con la misma búsqueda.
    (() => {
      const datos = filas.filter(f => f.tipo === 'dato' && f.total);
      for (let i = 0; i < datos.length; i++) {
        for (let j = i + 1; j < datos.length; j++) {
          if (Math.abs(datos[i].total - datos[j].total) > 0.005) continue;
          const igualMes = datos[i].meses.every((v, k) =>
            Math.abs((v || 0) - (datos[j].meses[k] || 0)) < 0.005);
          if (!igualMes) continue;
          avisos.push({
            grave: false,
            texto: '"' + datos[i].concepto + '" y "' + datos[j].concepto + '" traen exactamente ' +
                   'el mismo número todos los meses. Puede ser casualidad, pero suele significar ' +
                   'que los dos se llenaron con la misma fórmula y una de las dos búsquedas ' +
                   'encontró el renglón equivocado.'
          });
        }
      }
    })();
    ...
    if (repetidos.length) {
      if (sinDesempate) {
        avisos.push({
          grave: true,
          texto: 'Hay ' + repetidos.length + (repetidos.length === 1 ? ' concepto que aparece' :
            ' conceptos que aparecen') + ' de los dos lados (' + repetidos.join(' · ') + ') y ' +
            fmt(sinDesempate) + (sinDesempate === 1 ? ' renglón capturado no dice' :
            ' renglones capturados no dicen') + ' de cuál lado es. Esos renglones no se están ' +
            'contando. Escribe INGRESO o EGRESO en la columna de nota de esos renglones, o ' +
            'ponles nombres distintos.'
        });
      } else {
        avisos.push({
          grave: false,
          texto: repetidos.join(' · ') + (repetidos.length === 1 ? ' aparece' : ' aparecen') +
            ' de los dos lados del estado. Se están separando con lo que dice la columna de nota ' +
            'de cada renglón capturado (INGRESO o EGRESO), así que cada lado muestra lo suyo.'
        });
      }
    }
