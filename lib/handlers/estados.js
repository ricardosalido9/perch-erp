...
module.exports = async (req, res) => {
  try {
    ...
    (() => {
      ...
      const repes = Object.keys(cuenta).filter(k => cuenta[k] > 1);
      if (repes.length) {
        avisos.push({
          grave: true,
          texto: 'Hay ' + repes.length + (repes.length === 1 ? ' concepto repetido' : ' conceptos repetidos') +
                 ' en la estructura: ' + repes.join(' · ') + '. Los datos se guardan por año, mes y ' +
                 'concepto, así que dos renglones con el mismo nombre se pisan y los dos acaban ' +
                 'mostrando el mismo número. Hay que distinguirlos, por ejemplo "recibido" y "pagado".'
        });
      }
    })();
