...
module.exports = async (req, res) => {
  try {
    ...
    if (er.headers.length) {
      ...
      if (dA && dM && dC && dV) {
        hayER = true;
        const SUMAN = ['ingresos por venta de productos', 'ingresos por prestacion de servicios'];
        er.rows.forEach(r => {
          if (num(r[dA]) !== anio) return;
          const m = num(r[dM]);
          if (m < 1 || m > 12) return;
          const nc = norm(r[dC]);
          if (SUMAN.indexOf(nc) !== -1) capturado[m] += num(r[dV]);
        });
      }
    }
    ...
    return res.status(200).json({
      ...
      sinFecha,
      nota: 'VENTAS se compara contra "Ingresos por venta de productos" más "Ingresos por ' +
            'prestación de servicios" del estado. Si tu hoja de VENTAS lleva el total con envío ' +
            'y el estado no, la diferencia va a ser el envío y no un error.'
    });
  } catch (e) {
