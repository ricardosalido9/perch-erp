...
module.exports = async (req, res) => {
  try {
    ...
    const [vig, comp, egr] = await Promise.all([
    ...
    vig.rows.forEach(r => {
      ...
      const prov = txt(vEmisor ? r[vEmisor] : '');
      const base = {
        uuid: txt(r[vUUID]), proveedor: prov, rfc: txt(vRfcEm ? r[vRfcEm] : ''),
    ...
    if (egr.headers.length) {
      ...
      egr.rows.forEach(r => {
        ...
        const prov = txt(eP ? r[eP] : '');
        if (!prov) { sinProveedor++; return; }
        const k = norm(prov);
