  ...
  try {
    ...
    if (lista.headers.length) {
      ...
      if (lCom) lista.rows.forEach(r => {
        ...
        if (razones.length) razones.forEach(rz => { aComercial[norm(rz)] = com; });
    ...
    const sinFactura = Object.keys(porProveedor).map(k => {
      ...
      if (esMensual) {
        ...
        (p.facturas || []).forEach(f => {
          ...
          if (!mm) return;
          // Una factura de los primeros días del mes suele ser del mes anterior
          const dia = f.dia % 100;
          if (dia <= 10) {
            let a = Math.floor(mm / 100), m2 = mm % 100;
            m2 -= 1;
            if (m2 < 1) { m2 = 12; a -= 1; }
            const anterior = a * 100 + m2;
            if (meses[anterior] && !meses[mm]) mm = anterior;
          }
          if (!meses[mm]) meses[mm] = { mes: mm, pagado: 0, facturado: 0, nPagos: 0, nFacturas: 0 };
