...
module.exports = async (req, res) => {
  try {
    ...
    if (egr.headers.length) {
      ...
      const eMet = col(egr.headers, 'Método de pago', 'Metodo de pago', 'Forma de pago');
      ...
      egr.rows.forEach(r => {
        ...
        // Solo lo que salió del banco: el efectivo no se puede comprobar con CFDI
        const forma = norm(eMet ? r[eMet] : '') + ' ' + norm(eCta ? r[eCta] : '');
        formasVistas[txt(eMet ? r[eMet] : '') || '(vacío)'] =
          (formasVistas[txt(eMet ? r[eMet] : '') || '(vacío)'] || 0) + 1;
        if (/efectivo|caja chica|caja\b/.test(forma)) { enEfectivo++; efectivoMonto += m; return; }
    ...
    const sinFactura = Object.keys(porProveedor).map(k => {
      ...
      return Object.assign({}, p, {
        ...
        facturas: (p.facturas || []).slice(0, 30)
      });
