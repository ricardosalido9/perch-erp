...
const OBLIGATORIOS = ['Teléfono', 'Correo', 'Dirección', 'RFC'];
  ...
  try {
    ...
    if (cli.headers.length) {
      ...
      if (cN) cli.rows.forEach(r => {
        ...
        OBLIGATORIOS.forEach(o => {
          const c = col(cli.headers, o, o.replace('ó', 'o').replace('é', 'e'));
          if (c && !txt(r[c])) f.faltan.push(o);
          if (!c && f.faltan.indexOf(o) === -1) f.faltan.push(o);
        });
      });
    }
    ...
    if (shw.headers.length) {
      ...
      if (sN) shw.rows.forEach(r => {
        ...
        if (!f) return;
        f.visitas++;
        marca(f, sF ? fechaNum(r[sF]) : null, 'Visita al showroom', '');
      });
    }
