...
module.exports = async (req, res) => {
  try {
    ...
    // Se guarda el número de fila de cada renglón: es lo único que identifica un
    // renglón sin lugar a dudas. Antes se quitaban por POSICIÓN en la lista, y si
    // la pantalla tenía un filtro o un orden distinto al de la hoja, las
    // posiciones no coincidían y se quitaba el renglón equivocado —o ninguno—.
    const origen = [];
    for (let i = hr + 1; i < values.length; i++) {
      const f = values[i] || [];
      const o = { _fila: i + 1 }; H.forEach((h, j) => { o[h] = f[j]; });
    ...
    const nuevos = origen.map((o, i) => {
      ...
      // La fecha de la versión nueva es HOY.
      //
      // Estas columnas se excluyen de la copia para no heredar las de la
      // cotización original, pero nadie las volvía a llenar: la versión nueva
      // nacía sin fecha y el filtro de fechas de la pantalla la escondía. Se
      // guardaba bien y no aparecía por ningún lado.
      const hoy = new Date();
      const ponSi = (nombres, valor) => {
        for (const n of nombres) {
          const real = H.filter(h => norm(h) === norm(n))[0];
          if (real) { rec[real] = valor; return; }
        }
      };
      ponSi(['Fecha del Cierre', 'Fecha'], hoyLargo());
      ponSi(['Mes'], hoy.getMonth() + 1);
      ponSi(['Año', 'Ano'], hoy.getFullYear());
      ponSi(['Trimestre'], 'Q' + (Math.floor(hoy.getMonth() / 3) + 1));
      if (cCom) {
    ...
    // Renglones que se quitan. Se aceptan los números de fila —lo correcto— y,
    // por compatibilidad, las posiciones que mandaban las pantallas viejas.
    const quitarFilas = (body.quitarFilas || []).map(x => String(x));
    const quitar = (body.quitar || []).map(x => String(x));
    const aEscribir = nuevos.filter((rec, i) => {
      if (quitarFilas.length) {
        return quitarFilas.indexOf(String(origen[i] && origen[i]._fila)) === -1;
      }
      return quitar.indexOf(String(i)) === -1;
    });
