...
async function addRecordsBatch(key, records) {
  ...
  for (let n = 0; n < prepared.length; n++) {
    ...
    await writeRowSkipping(cfg.id, cfg.sheetName, target + n, rowArr, skip);
  }
  ...
  ventas_registro: [
    {
      ...
      // { field: nombre del campo en VENTAS, from: nombre(s) de la columna en el CATÁLOGO }
      levels: [
        { field: 'Producto',       from: ['Productos', 'Producto', 'Nombre'] },
        { field: 'Material',        from: ['Material', 'Coleccion', 'Colección'] },  // la madera (Nogal, Tzalam…)
        { field: 'Material Extra',  from: ['Material Extra', 'Material 2'] }          // 2º eje (Tela, Kraft, Acero, Bolsa…)
      ],
      fills: [
        ...
        { field: 'Costo Unitario',   from: ['Costo Unitario', 'Costos Total', 'Costo Total', 'Costos', 'Costo'] }
      ]
...
async function _resolveCascade(cfgArea, lk) {
  ...
  // Cada nivel puede ser un string, o { field: 'ColEnVentas', from: ['ColEnCatalogo', ...alias] }
  const levelCols = (lk.levels || []).map(l => {
    const field = (typeof l === 'string') ? l : l.field;
    const from  = (typeof l === 'string') ? l : (l.from || l.field);
    return { name: field, col: findCol(from) };
  }).filter(x => x.col !== -1);
  if (!levelCols.length) return null;
  // Igual para los campos autocompletados: string, o { field: 'ColEnVentas', from: ['ColEnCatalogo', ...] }
