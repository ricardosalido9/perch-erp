...
async function addRecord(key, record) {
  ...
  // Continúa las fórmulas de la fila de arriba en las columnas calculadas
  try { await copiarFormulas(cfg.id, sheetName, target - 1, target, skip); } catch (e) { /* la fila queda igual */ }
...
async function addRecordsBatch(key, records) {
  ...
  for (let n = 0; n < prepared.length; n++) {
    ...
    try { await copiarFormulas(cfg.id, sheetName, target + n - 1, target + n, skip); } catch (e) { /* sigue */ }
  }
