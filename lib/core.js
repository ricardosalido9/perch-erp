...
const COLUMNAS_SUGERIDAS = {
  op_envios: ['Fecha', 'Mes', 'Año', 'Pedido', 'Cliente', 'Status',
              ...
              'Costo del Envío', 'Costo del Envío c / IVA', 'Guía', 'Ticket de Remisión',
              'Quién lo paga', 'Comentarios'],
  ...
  op_envios: [
    {
      ...
      // Las piezas del folio se suman: un pedido puede tener varios muebles
      sumar: { campo: 'Piezas', col: ['Cantidad'] }
    }
  ],
...
async function _resolveLookup(cfgArea, lk) {
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    if (lk.sumar) {
      const cS = findCol(lk.sumar.col || [lk.sumar.campo]);
      if (cS !== -1) {
        const n = parseFloat(String(row[cS] == null ? '' : row[cS]).replace(/[^0-9.\-]/g, ''));
        if (!isNaN(n)) {
          const prev = map[name] ? parseFloat(map[name][lk.sumar.campo] || 0) || 0 : 0;
          rec[lk.sumar.campo] = Math.round((prev + n) * 100) / 100;
        }
      }
    }
  ...
  if (lk.sumar) fillTargets.push(lk.sumar.campo);
