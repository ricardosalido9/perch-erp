...
const MENU = [
  ...
  { key:'ventas',      label:'Ventas',      icon:'tag', children:[
      ...
      // Folio por folio: qué se vendió y si ya se le pidió al proveedor
      { key:'ventas_folios',   label:'Ventas por folio' },
      { key:'leads',           label:'Leads' },
      { key:'showroom',        label:'Showroom' },
      { key:'expediente',      label:'Historial de clientes' },
  ...
  { key:'produccion',  label:'Producción',  icon:'tool', children:[
      ...
      { key:'pagos_pedidos', label:'Pagos por hacer' },
  ...
  { key:'facturacion', label:'Facturación e impuestos', icon:'receipt', children:[
      { key:'fac_emision', label:'Emisión', children:[
          { key:'fac_control',  label:'Facturas y complementos' },
          { key:'cfdi_control', label:'CFDIs y complementos' },
          { key:'fac_emitidos', label:'Emitidos vs ingresos' },
          // La pestaña COMPLEMENTOS EMITIDOS ya existe en el archivo de CFDIs
          { key:'fac_comp_emitidos', label:'Complementos emitidos' }
      ] },
      { key:'fac_recibidos', label:'Recibidos', children:[
          { key:'cfdi_prov',  label:'Qué me deben los proveedores' },
          // El mismo cruce que en emisión, del otro lado: qué factura recibida
          // sigue sin que el proveedor nos mande su complemento.
          { key:'cfdi_comp_rec', label:'CFDIs y complementos recibidos' },
          { key:'prov_razon', label:'Lista de Proveedores' }
      ] },
      { key:'fac_imp', label:'Impuestos', children:[
          { key:'fac_impuestos', label:'Impuestos por mes' }
      ] }
  ] },
  { key:'logistica', label:'Entregas', icon:'truck', children:[
      { key:'log_calendario', label:'Calendario' }
  ] },
  { key:'rh', label:'Nómina y RH', icon:'users', children:[
      { key:'rh_resumen',  label:'Resumen' },
      { key:'rh_personal', label:'Colaboradores' },
      { key:'rh_nomina',   label:'Nómina quincenal' }
  ] },
  // Finanzas se agrupa por el estado financiero al que alimenta cada área, no por
  // el tipo de pantalla. Así, quien captura sabe a dónde va lo que está capturando:
  // los saldos arman el balance, los movimientos arman el flujo, y el resultado
  // sale de los dos.
  { key:'finanzas',    label:'Finanzas',    icon:'dollar', children:[
      { key:'fin_g_balance', label:'Balance', children:[
          { key:'fin_balance',  label:'Balance general' },
          { key:'fin_bancos',   label:'Bancos y Cajas' },
          { key:'fin_cxc',      label:'Cuentas por Cobrar' },
          // Cuentas por pagar es lo mismo que "Pagos por hacer" de Producción:
          // lo que se le debe a proveedores. Es la misma pantalla, puesta también
          // aquí porque es un renglón del balance.
          { key:'pagos_pedidos', label:'Cuentas por Pagar a proveedores' },
          { key:'cta_clientes', label:'Estado de cuenta clientes' }
      ] },
      { key:'fin_g_flujo', label:'Flujo de Efectivo', children:[
          { key:'fin_flujo',    label:'Flujo de efectivo' },
          { key:'fin_ingresos', label:'Ingresos' },
          { key:'fin_egresos',  label:'Egresos' },
          { key:'fin_banco',    label:'Subir banco' },
          { key:'fin_efectivo', label:'Efectivo' },
          { key:'fin_gastos_op',label:'Gastos operativos' }
      ] },
      { key:'fin_estados', label:'Estado de Resultados' }
  ] }
];
...
const SHEETS = {
  ...
  ventas_registro: { id: CFG.ARCHIVOS.VENTAS, sheetName: CFG.PESTANAS.ventas },
  // Leads: quien escribe pero todavía no pide cotización. Captura mínima a propósito.
  leads:           { id: CFG.ARCHIVOS.VENTAS, sheetName: CFG.PESTANAS.leads },
  // Showroom: la visita existe por sí sola, venga o no de una cotización
  showroom:        { id: CFG.ARCHIVOS.VENTAS, sheetName: CFG.PESTANAS.showroom },
  // Cotizaciones: MISMA estructura que VENTAS (duplicar la pestaña y renombrarla)
  cotizaciones:    { id: CFG.ARCHIVOS.VENTAS, sheetName: CFG.PESTANAS.cotizaciones },
  ventas_clientes: { id: CFG.ARCHIVOS.CLIENTES, sheetName: CFG.PESTANAS.clientes },
  ...
  compras_proveedores: { id: CFG.ARCHIVOS.PROVEEDORES, sheetName: CFG.PESTANAS.proveedores },
  ...
  inventario: { id: CFG.ARCHIVOS.CATALOGO, sheetName: 'Lista de Precios 2026',
                sheetNamePattern: 'Lista de Precios {AAAA}', headerRow: 2 },
  // --- Marketing (misma base de Ventas, otra pestaña; se usa en el Dashboard) ---
  marketing: { id: CFG.ARCHIVOS.VENTAS, sheetName: CFG.PESTANAS.marketing },
  // --- Metas mensuales (se usa en el Dashboard) ---
  metas: { id: CFG.ARCHIVOS.VENTAS, sheetName: CFG.PESTANAS.metas },
  // --- Funnel de ventas / cotizaciones ---
  // La fila 1 de esta pestaña trae una celda suelta ("q"); los encabezados reales están en la FILA 2.
  funnel: { id: CFG.ARCHIVOS.FUNNEL, sheetName: CFG.PESTANAS.funnel, headerRow: 2 },
  // --- Finanzas (conectar cuando estén las hojas) ---
  // Bancos y Cajas no lee una hoja: calcula el saldo de cada cuenta desde
  // INGRESOS y EGRESOS más el arranque capturado en BG Datos.
  fin_cxc:      { id: CFG.ARCHIVOS.VENTAS, sheetName: CFG.PESTANAS.cxc },
  // fin_cxp:      { id: '', sheetName: 'Cuentas por Pagar' },
  fin_ingresos: { id: CFG.ARCHIVOS.FINANZAS, sheetName: CFG.PESTANAS.ingresos },
  // Ingresos y Egresos viven en el archivo histórico de finanzas
  fin_egresos:  { id: CFG.ARCHIVOS.FINANZAS, sheetName: CFG.PESTANAS.egresos },
  ...
  prov_pedidos:  { id: CFG.ARCHIVOS.PRODUCCION, sheetName: CFG.PESTANAS.pedidos },
  // La pestaña de entradas se llama "Entradas" en el Consolidado, pero en otros
  // clientes viene como "Entradas de Inventario". Se prueban los dos nombres.
  prov_entradas: { id: CFG.ARCHIVOS.PRODUCCION, sheetName: CFG.PESTANAS.entradas,
                   sheetNameAlt: ['Entradas', 'Entradas de Inventario', 'ENTRADAS'] },
  prov_salidas:  { id: CFG.ARCHIVOS.PRODUCCION, sheetName: CFG.PESTANAS.salidas },
  op_stock:      { id: CFG.ARCHIVOS.PRODUCCION, sheetName: CFG.PESTANAS.stock },
  op_revisar:    { id: CFG.ARCHIVOS.PRODUCCION, sheetName: CFG.PESTANAS.revisar },
  // Pantalla propia: sube el PDF del banco y propone la captura. No lee una
  // hoja directa, pero necesita existir para que el menú y los permisos la vean.
  fin_banco:     { id: CFG.ARCHIVOS.FINANZAS, sheetName: CFG.PESTANAS.ingresos },
  // Pantalla propia: facturas emitidas contra complementos y contra ingresos
  // Los tres juegos de CFDIs viven en el mismo archivo, cada uno en su pestaña.
  // Los nombres viejos se dejan como alternativa: si el archivo todavía no está
  // migrado, el ERP los encuentra igual en vez de salir vacío.
  fac_emitidos:  { id: CFG.ARCHIVOS.CFDI_EMITIDOS, sheetName: CFG.PESTANAS.cfdiEmitidos,
                   sheetNameAlt: ['EMITIDOS', 'VIGENTES'] },
  // CFDIs emitidos: la verdad del SAT sobre facturas y complementos
  cfdi_vigentes: { id: CFG.ARCHIVOS.CFDI_EMITIDOS, sheetName: CFG.PESTANAS.cfdiEmitidos,
                   sheetNameAlt: ['EMITIDOS', 'VIGENTES'] },
  // CFDIs que nos emiten los proveedores
  cfdi_recibidos: { id: CFG.ARCHIVOS.CFDI_RECIBIDOS, sheetName: CFG.PESTANAS.cfdiRecibidos,
                   sheetNameAlt: ['RECIBIDOS', 'VIGENTES'] },

  // Envíos: qué se mandó, con quién, a dónde y cuánto costó
  op_envios:     { id: CFG.ARCHIVOS.PRODUCCION, sheetName: CFG.PESTANAS.envios },
  // Bitácora de entregas y recolecciones: lo que marca logística
  log_movimientos: { id: CFG.ARCHIVOS.PRODUCCION, sheetName: CFG.PESTANAS.logistica },

  // El calendario de entregas y recolecciones: vista calculada, no una hoja
  log_calendario: { id: '', sheetName: '' },

  // Lo que Nico pide pagar. Hoy vive en Gastos Manuales; después se mueve al
  // archivo de Operación nuevo cambiando solo estas dos líneas.
  pagos_pedidos: { id: CFG.ARCHIVOS.OPERACION, sheetName: CFG.PESTANAS.gastosManuales },

  // La lista de costos por proveedor: sirve para saber quién puede fabricar un mueble
  costos_unitarios: { id: CFG.ARCHIVOS.CATALOGO, sheetName: CFG.PESTANAS.costosUnitarios },
  // Revisión manual de estados de cuenta: la hoja que lleva Vale a mano.
  // Vive en el CONSOLIDADO, no en Operación 2026.
  prov_revision: { id: CFG.ARCHIVOS.PRODUCCION, sheetName: CFG.PESTANAS.revision },
  ...
  // Un renglón por año, mes y concepto. La estructura del estado (qué es dato,
  // qué es suma y qué es fórmula) vive en la pestaña EF Conceptos del mismo archivo.
  fin_estados:      { id: CFG.ARCHIVOS.ESTADOS, sheetName: CFG.PESTANAS.erDatos },
  fin_er_conceptos: { id: CFG.ARCHIVOS.ESTADOS, sheetName: CFG.PESTANAS.erConceptos },
  fin_flujo:        { id: CFG.ARCHIVOS.ESTADOS, sheetName: CFG.PESTANAS.efDatos },
  fin_ef_conceptos: { id: CFG.ARCHIVOS.ESTADOS, sheetName: CFG.PESTANAS.efConceptos },
  // El balance usa las mismas dos pestañas por estado. Todavía no existen: el área
  // lo dice en pantalla en vez de salir vacía.
  fin_balance:      { id: CFG.ARCHIVOS.ESTADOS, sheetName: CFG.PESTANAS.bgDatos },
  fin_bg_conceptos: { id: CFG.ARCHIVOS.ESTADOS, sheetName: CFG.PESTANAS.bgConceptos },

  // ===== Nómina y RH =====
  // Lista de colaboradores: quién trabaja aquí, con su sueldo y datos fiscales
  // Un renglón sin nombre en Colaboradores no es una persona: es un resto de la
  // hoja, una fórmula que devolvió vacío o un renglón de separación. Se esconden
  // para que la lista sea la gente y nada más.
  rh_personal:   { id: CFG.ARCHIVOS.COLABORADORES, sheetName: CFG.PESTANAS.colaboradores,
                   requiere: ['Nombre', 'Colaborador', 'Nombre del colaborador'] },
  // Nómina corriente: lo que se paga cada semana
  // La pestaña se llama NOMINA a secas. Se aceptan los nombres viejos con año
  // por si algún archivo todavía no se ha migrado.
  rh_nomina:     { id: CFG.ARCHIVOS.NOMINA, sheetName: CFG.PESTANAS.nomina,
                   sheetNameAlt: ['NOMINA', 'Nómina', 'Nomina',
                                  'Nómina 2026', 'Nomina 2026', 'Nómina 2027'] },
  // Los complementos de pago, cada lado en su pestaña. Todavía no existen: hasta
  // que se creen, quien las lea recibe vacío y lo dice en pantalla.
  // Las facturas que nos emitieron, para el cruce contra sus complementos
  cfdi_prov_vigentes: { id: CFG.ARCHIVOS.CFDI_RECIBIDOS, sheetName: CFG.PESTANAS.cfdiRecibidos,
                        sheetNameAlt: ['RECIBIDOS', 'Recibidos', 'RECIBIDAS'] },
  fac_comp_emitidos:  { id: CFG.ARCHIVOS.CFDI_EMITIDOS, sheetName: CFG.PESTANAS.compEmitidos,
                        sheetNameAlt: ['COMPLEMENTOS EMITIDOS', 'COMPLEMENTOS DE PAGO'] },
  fac_comp_recibidos: { id: CFG.ARCHIVOS.CFDI_RECIBIDOS, sheetName: CFG.PESTANAS.compRecibidos,
                        sheetNameAlt: ['COMPLEMENTOS RECIBIDOS', 'COMPLEMENTOS DE PAGO'] },
};
...
const COLUMNAS_SUGERIDAS = {
  ...
  fin_estados:      ['Año', 'Mes', 'Estado', 'Concepto', 'Monto', 'Origen', 'Notas'],
  fin_flujo:        ['Año', 'Mes', 'Estado', 'Concepto', 'Monto', 'Origen', 'Notas'],
  fin_balance:      ['Año', 'Mes', 'Estado', 'Concepto', 'Monto', 'Origen', 'Notas'],
  fin_bg_conceptos: ['Orden', 'Estado', 'Concepto', 'Nivel', 'Padre', 'Tipo',
                     'Cómo se calcula', 'Signo'],
  fin_er_conceptos: ['Orden', 'Estado', 'Concepto', 'Nivel', 'Padre', 'Tipo',
                     'Cómo se calcula', 'Signo'],
  fin_ef_conceptos: ['Orden', 'Estado', 'Concepto', 'Nivel', 'Padre', 'Tipo',
                     'Cómo se calcula', 'Signo'],
...
async function resolveSheetName(cfg) {
  if (!cfg) return null;
  // La misma pestaña se llama distinto según el cliente ("Entradas" o "Entradas de
  // Inventario"). Se busca cuál de los nombres existe de verdad y se usa ese, tanto
  // para leer como para escribir. Si no existe ninguno, se deja el de siempre para
  // que el error diga qué pestañas SÍ hay.
  if (cfg.sheetNameAlt && cfg.sheetNameAlt.length && cfg.id) {
    const cka = cfg.id + '|alt|' + cfg.sheetName;
    if (_resolvedCache[cka]) return _resolvedCache[cka];
    let titulos;
    try { titulos = await listTabs(cfg.id); } catch (e) { return cfg.sheetName; }
    const posibles = [cfg.sheetName].concat(cfg.sheetNameAlt);
    for (const nombre of posibles) {
      const real = titulos.filter(t => _norm(t) === _norm(nombre))[0];
      if (real) { _resolvedCache[cka] = real; return real; }
    }
...
// La fila donde están los encabezados, en base 0.
//
// Si el área la declara, se usa esa. Si no, se busca: la primera fila con al
// menos tres celdas escritas. Hay hojas —la Lista de Clientes— que traen un
// bloque de título arriba y los encabezados más abajo; leerlos de la fila 1
// daba encabezados vacíos, y al guardar un cambio no había a qué columna
// escribirlo, así que la edición "no se guardaba" sin dar error.
//
// Lectura y escritura DEBEN usar esta misma función: si una encuentra la fila 19
// y la otra la 1, se lee una cosa y se escribe otra.
function _hrIdx(cfg, values) {
  if (cfg && cfg.headerRow && cfg.headerRow > 1) return cfg.headerRow - 1;
  if (!values || !values.length) return 0;
  const conDatos = (fila) => (fila || []).filter(c => String(c == null ? '' : c).trim() !== '').length;
  // La fila 1 con suficientes datos es el caso normal: se respeta
  if (conDatos(values[0]) >= 3) return 0;
  for (let i = 1; i < Math.min(values.length, 40); i++) {
    if (conDatos(values[i]) >= 3) return i;
  }
  return 0;
}
...
const PERMISOS = {
  comercial: {
    // El catálogo es un menú propio, no un hijo de ventas: si no está aquí, no
    // aparece aunque el área esté permitida más abajo.
    menus: ['inicio', 'dashboard', 'ventas', 'inventario', 'finanzas'],
  ...
  operativo: {
    // Se le da el menú de facturación, pero dentro solo sobrevive "Qué me deben los
    // proveedores": el menú se poda solo con las áreas de abajo. Y esa pantalla, para
    // este rol, viene filtrada a los proveedores que aparecen en pedidos de producción,
    // así que no ve a los administrativos (renta, contador, software).
    // Ventas por folio sí la ve: la necesita para saber qué se vendió y qué falta
    // pedir. Va sin dinero —el handler le quita el total a este rol— porque el
    // precio de venta no es asunto de producción.
    menus: ['inicio', 'produccion', 'facturacion', 'ventas'],
    areas: ['inicio', 'prov_pedidos', 'prov_entradas', 'prov_salidas', 'op_stock', 'op_revisar',
            'proveedores', 'inv_prov', 'inventario', 'op_envios', 'cfdi_prov', 'ventas_folios',
            // Comparativo de costos en vez del registro de ventas: ahí se ve qué
            // pidió cada venta, de dónde sale el costo de cada pieza, el valor de
            // la venta y el margen. Es lo mismo que necesitaba, mejor armado.
            'costos'],
    ocultarColumnas: []
  },
  // Logística: el calendario de su ruta. Marca lo revisado, lo entregado y lo
  // recogido, y deja comentarios. No ve precios ni costos.
  // (Estaba escrito dos veces: la segunda pisaba a la primera y se perdían tres
  // columnas de la lista de ocultas. Quedan unidas.)
  logistica: {
    menus: ['logistica'],
    areas: ['log_calendario'],
    ocultarColumnas: ['Precio Unitario', 'Costo Unitario', 'Costo total', 'Costo Total',
                      'Total con IVA', 'Total con envio sin impuestos', 'Utilidad',
                      '% Utilidad', 'Margen', 'Costos Total', '% descuento', 'Descuentos']
  },
  // Fiscal: facturación, impuestos, nómina y la cobranza. No ve producción ni costos.
  fiscal: {
    menus: ['inicio', 'facturacion', 'rh', 'finanzas'],
    areas: ['inicio',
            'fac_control', 'cfdi_control', 'cfdi_comp_rec', 'cfdi_prov', 'prov_razon', 'fac_impuestos', 'fin_banco',
            'fac_emitidos',
            'rh_resumen', 'rh_personal', 'rh_nomina',
            'fin_cxc', 'cta_clientes', 'fin_ingresos', 'fin_egresos', 'fin_efectivo',
            'fin_estados', 'fin_flujo', 'fin_balance', 'fin_gastos_op',
            'fac_comp_emitidos', 'fac_comp_recibidos', 'pagos_pedidos',
            'ventas_clientes', 'pagos_pedidos', 'ventas_folios'],
    ocultarColumnas: []
  }
};
function permisosDe(rol) {
  // Sin acentos y en minúsculas: "Logística" y "Logistica" son el mismo rol
...
const _MESES_REF = { ene:1, feb:2, mar:3, abr:4, may:5, jun:6, jul:7, ago:8, sep:9, sept:9, oct:10, nov:11, dic:12, enero:1,febrero:2,marzo:3,abril:4,mayo:5,junio:6,julio:7,agosto:8,
...
// Agrega filas al final de una pestaña, sin tocar lo que ya está.
// Si la pestaña no existe, se crea con los encabezados que se le pasen.
async function appendRows(spreadsheetId, nombre, filas, encabezados) {
  ...
  if (encabezados && encabezados.length) {
    const actual = await readRange(spreadsheetId, real).catch(() => []);
    if (!actual.length || !(actual[0] || []).some(x => String(x).trim())) {
      await sheets.spreadsheets.values.update({
...
// folioFijo: cuando se agrega un producto a un pedido que YA existe. Sin esto se
// genera un folio nuevo y el producto acaba en otra cotización, que es lo que
// pasaba con el botón "Agregar producto".
async function addRecordsBatch(key, records, folioFijo) {
  const cfg = SHEETS[key];
  if (!cfg) throw new Error('Esta área no está conectada.');
  ...
  if (ar) {
    ...
    if (iRef !== -1) {
      folio = folioFijo
        ? String(folioFijo).trim()
        : _generarFolio(values, headers, ar.field, prepared[0], ar.dateField, ar.prefix);
      const actual = headers[iRef];
...
const LOOKUPS = {
  ventas_registro: [
    {
      ...
      type: 'cascade',
      id: CFG.ARCHIVOS.CATALOGO,   // archivo "Lista de Precios 2026"
    ...
    {
      id: CFG.ARCHIVOS.CLIENTES,  // archivo "Lista de Clientes"
      sheetName: CFG.PESTANAS.clientes,
  ...
  leads: [
    { id: CFG.ARCHIVOS.CLIENTES, sheetName: CFG.PESTANAS.clientes,
      keyField: 'Contacto', keyAliases: ['Cliente', 'Nombre/Razón Social', 'Nombre'],
      fills: [{ from: ['Despacho'], to: 'Despacho' }] },
    // El despacho se puede elegir solo, sin escribir primero el nombre
    { id: CFG.ARCHIVOS.CLIENTES, sheetName: CFG.PESTANAS.clientes,
      keyField: 'Despacho', keyAliases: ['Despacho'], fills: [] }
  ],
  showroom: [
    { id: CFG.ARCHIVOS.CLIENTES, sheetName: CFG.PESTANAS.clientes,
  ...
  op_envios: [
    {
      ...
      type: 'lookup',
      id: CFG.ARCHIVOS.VENTAS,
      sheetName: CFG.PESTANAS.ventas,
      keyField: 'Pedido',
  ...
  // Entradas: el formulario de alta no tenía ni un desplegable, así que todo se
  // escribía a mano —proveedor, pedido, mueble, material— y cualquier dedazo dejaba
  // la entrada sin poder ligarse a su pedido. Ahora se elige en cascada desde los
  // pedidos que ya existen: proveedor, luego SUS pedidos, luego los muebles de ESE
  // pedido. El folio del cliente y la fecha se traen solos.
  prov_entradas: [
    {
      type: 'cascade',
      id: CFG.ARCHIVOS.PRODUCCION,
      sheetName: CFG.PESTANAS.pedidos,
      levels: [
        { field: 'Proveedor',       from: ['Proveedor'] },
        { field: 'Pedido Proveedor',from: ['Pedido Proveedor', 'Pedido'] },
        { field: 'Item',            from: ['Productos', 'Producto', 'Item'] },
        { field: 'Material',        from: ['Material'] }
      ],
      // Lo que se copia del renglón del pedido una vez elegido el mueble. Solo
      // se llenan los campos que existan en Entradas; los demás se ignoran.
      fills: ['Folio cliente']
    }
  ],
  prov_pedidos: [
    ...
    {
      id: CFG.ARCHIVOS.PROVEEDORES,
      sheetName: CFG.PESTANAS.proveedores,
      keyField: 'Proveedor',
      keyAliases: ['Nombre/Razón Social', 'Razón Social', 'Proveedor'],
      // Cuánto tarda normalmente ese proveedor. Con eso el pedido propone la fecha
      // estimada de entrega, que se queda editable por si hay una urgencia.
      // Si la columna no existe todavía en la Lista de Proveedores, no pasa nada:
      // el campo simplemente no se propone.
      fills: [{ from: ['Días de entrega', 'Dias de entrega', 'Días de Entrega',
                       'Tiempo de entrega', 'Días', 'Dias'], to: '_DiasEntrega' }]
    },
    {
      ...
      // Vive en el archivo del CATÁLOGO (antes el código la buscaba en Operación 2026,
      // donde ya no está). Se prueban los tres archivos por si cambia de casa.
      type: 'tabla',
      id: CFG.ARCHIVOS.CATALOGO,
      idAlt: [CFG.ARCHIVOS.PRODUCCION, CFG.ARCHIVOS.OPERACION],
      sheetName: CFG.PESTANAS.costosUnitarios,
      llaves: [
  ...
  compras_registro: [
    {
      sheetName: CFG.PESTANAS.proveedores,        // completá el id en SHEETS.compras_proveedores
...
async function _resolveTabla(cfgArea, lk) {
  // La pestaña puede vivir en distintos archivos según el cliente: se prueban en
  // orden y se usa el primero que sí la tenga, en vez de fallar en silencio.
  const ids = [lk.id || cfgArea.id].concat(lk.idAlt || []).filter(Boolean);
  for (let n = 0; n < ids.length; n++) {
    const r = await _resolveTablaEn(ids[n], lk);
    if (r && r.filas && r.filas.length) return r;
  }
  return null;
}
async function _resolveTablaEn(id, lk) {
  ...
  if (!values.length) return null;
  // Los encabezados pueden estar en la fila 1 o más abajo si la pestaña trae un título.
  // Se busca el primer renglón (de los tres primeros) que contenga alguna de las
  // columnas llave, en vez de dar por hecho que es el primero.
  let hr = (lk.headerRow && lk.headerRow > 1) ? (lk.headerRow - 1) : 0;
  if (!lk.headerRow) {
    const busca = [].concat.apply([], (lk.llaves || []).map(k => k.col || [k.campo]));
    for (let f = 0; f < Math.min(3, values.length); f++) {
      const fila = (values[f] || []).map(h => _norm(h));
      if (busca.some(c => fila.indexOf(_norm(c)) !== -1)) { hr = f; break; }
    }
  }
  const headers = (values[hr] || []).map(h => String(h).trim());
  const idx = (nombres) => {
  ...
  return {
    ...
    conAnio: iAnio !== -1,
    // De dónde salió y qué encontró: es lo que se ve en ?action=lookup-debug
    origen: {
      archivo: id, pestana: hoja, filaDeEncabezados: hr + 1, encabezados: headers,
      llavesEncontradas: llaves.map(k => k.campo + ' → ' + headers[k.i]),
      llavesFaltantes: (lk.llaves || []).map(k => k.campo)
        .filter(c => !llaves.some(k => k.campo === c)),
      rellenosEncontrados: rellenos.map(f => f.field + ' → ' + headers[f.i]),
      rellenosFaltantes: (lk.fills || []).map(f => (typeof f === 'string') ? f : f.field)
        .filter(c => !rellenos.some(f => f.field === c)),
      conAnio: iAnio !== -1
    },
    filas: filas
  };
...
module.exports = { MENU, SHEETS, USERS_SHEET, FORMULA_FIELDS, AREA_ROW_FILTERS, readRange, appendRow, updateRow,
  getDrive, copiarFormulas, writeCells, escribirTabla, asegurarPestana, appendRows, COLUMNAS_SUGERIDAS,
  PERMISOS, permisosDe, puedeVerArea,
  resolveSheetName, areaCfg, listTabs,
  writeRowSkipping, appendRows, _hrIdx,
  addRecordsBatch,
