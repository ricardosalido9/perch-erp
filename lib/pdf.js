...
async function generarCotizacion(cot, opciones) {
  ...
  const derecha = (t, xIzq, ancho, yy, font, size, color) => {
    const w = font.widthOfTextAtSize(t, size);
    page.drawText(t, { x: xIzq + ancho - w, y: yy, size, font, color: color || TINTA });
  };
  ...
  function nuevaPagina() {
    ...
    page.drawText(web, { x: A4.w - M - neg.widthOfTextAtSize(web, 8), y: y - 22, size: 8, font: neg, color: TINTA });
    y -= 56;
    raya(y, M, A4.w - M, 0.8);
    y -= 22;
    if (pagina === 1) {
      page.drawText('CLIENTE: ' + (cot.cliente || ''), { x: M, y, size: 9.5, font: reg, color: TINTA });
      if (cot.despacho) {
        page.drawText('DESPACHO: ' + cot.despacho, { x: M, y: y - 14, size: 9.5, font: reg, color: TINTA });
      }
  ...
  cot.items.forEach((it, idx) => {
    ...
    const anchoTxt = 118;
    const lineas = [];
    if (it.categoria) lineas.push({ t: it.categoria, f: reg, s: 7.5, c: SUAVE });
    ...
    lineas.forEach(l => {
      ...
      page.drawText(l.t, { x: M + (anchoTxt - w) / 2, y: ty, size: l.s, font: l.f, color: l.c });
      ty -= l.s + 3.5;
    });
  ...
  const yTotales = y;
  filas.forEach(f => {
    page.drawText(f[0], { x: xT, y, size: 8.5, font: f[2] ? neg : reg, color: TINTA });
  ...
  if (cond.length) {
    let yc2 = yTotales;
    page.drawText('CONDICIONES ESPECIALES', { x: M, y: yc2, size: 8, font: neg, color: TINTA });
    yc2 -= 14;
    cond.forEach(c => {
      cortar(c, reg, 7.5, ANCHO * 0.52).forEach(l => {
        page.drawText(l, { x: M, y: yc2, size: 7.5, font: reg, color: SUAVE });
        yc2 -= 10;
      });
    });
    if (o.banco) {
      yc2 -= 8;
      page.drawText(o.banco.titular + ' · ' + o.banco.banco, { x: M, y: yc2, size: 7, font: reg, color: SUAVE });
      yc2 -= 9;
      page.drawText('CLABE ' + o.banco.clabe, { x: M, y: yc2, size: 7, font: reg, color: SUAVE });
    }
  }
  ...
  if (o.terminos && o.terminos.length) {
    ...
    page.drawText(web2, { x: A4.w - M - neg.widthOfTextAtSize(web2, 7.5), y: y - 18, size: 7.5, font: neg, color: TINTA });
    y -= 44;
    const tit = 'CONDICIONES GENERALES DE VENTA' + (cot.folio ? '  ' + cot.folio : '');
    page.drawText(tit, { x: M, y, size: 9, font: neg, color: TINTA });
    y -= 8;
    ...
    o.terminos.forEach(bloque => {
      ...
      page.drawText(bloque.titulo, { x: colX, y, size: 7.8, font: neg, color: TINTA });
      y -= 12;
      bloque.puntos.forEach(p2 => {
        cortar('- ' + p2, reg, 6.9, COLW).forEach((l, i2) => {
          page.drawText(l, { x: colX + (i2 ? 6 : 0), y, size: 6.9, font: reg, color: SUAVE });
          y -= 8.6;
        });
      });
      y -= 9;
    ...
    if (o.banco) {
      page.drawText('DATOS BANCARIOS · ' + o.banco.banco, { x: M, y: yb, size: 7.6, font: neg, color: TINTA });
      yb -= 10;
      page.drawText(o.banco.titular, { x: M, y: yb, size: 6.8, font: reg, color: SUAVE }); yb -= 9;
      if (o.banco.cuenta) { page.drawText('CUENTA  ' + o.banco.cuenta, { x: M, y: yb, size: 6.8, font: reg, color: SUAVE }); yb -= 9; }
      page.drawText('CLABE  ' + o.banco.clabe, { x: M, y: yb, size: 6.8, font: reg, color: SUAVE }); yb -= 9;
      if (o.banco.swift) { page.drawText('SWIFT  ' + o.banco.swift, { x: M, y: yb, size: 6.8, font: reg, color: SUAVE }); yb -= 9; }
    ...
    if (o.facturacion) {
      ...
      page.drawText('FACTURA TU PEDIDO', { x: xf, y: yf, size: 7.6, font: neg, color: TINTA }); yf -= 10;
      o.facturacion.forEach(l => {
        cortar(l, reg, 6.8, ANCHO / 2 - 10).forEach(x2 => {
          page.drawText(x2, { x: xf, y: yf, size: 6.8, font: reg, color: SUAVE }); yf -= 9;
        });
      });
    }
  ...
  paginas.forEach((p, i) => {
    ...
    p.drawText(t, { x: (A4.w - w) / 2, y: M - 18, size: 7, font: reg, color: SUAVE });
  });
