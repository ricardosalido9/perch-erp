...
function limpio(v) {
  // Las ligaduras tipográficas (ﬀ, ﬁ, ﬂ) se expanden: la tipografía no las tiene
  // y al quitarlas quedaba un hueco, que se leía como "Coff ee".
...
// El id de un archivo de Drive, venga como venga la liga
function idDeDrive(url) {
  const u = String(url || '');
  let m = u.match(/\/file\/d\/([A-Za-z0-9_-]{20,})/);      // .../file/d/ID/view
  if (m) return m[1];
  m = u.match(/[?&]id=([A-Za-z0-9_-]{20,})/);                // ...?id=ID
  if (m) return m[1];
  m = u.match(/\/d\/([A-Za-z0-9_-]{20,})/);                 // .../d/ID/...
  if (m) return m[1];
  return null;
}

// Baja la foto de un producto.
//
// Las ligas del catálogo son de COMPARTIR, no de imagen: terminan en "/view" y lo
// que devuelven es una página HTML de Drive, no el archivo. Por eso ninguna foto
// salía en la cotización aunque la liga estuviera bien puesta.
//
// Cuando la liga es de Drive se baja con la cuenta de servicio, que además
// funciona con archivos que no son públicos.
async function bajarImagen(url) {
  if (!url || !/^https?:\/\//i.test(url)) return null;
  const id = /drive\.google\.com|docs\.google\.com/.test(url) ? idDeDrive(url) : null;

  if (id) {
    try {
      const core = require('./core');
      const d = core.getDrive();
      const meta = await d.files.get({ fileId: id, fields: 'mimeType',
                                       supportsAllDrives: true });
      const tipo = String((meta.data && meta.data.mimeType) || '').toLowerCase();
      if (tipo.includes('webp') || tipo.includes('svg')) return null;
      const r = await d.files.get({ fileId: id, alt: 'media', supportsAllDrives: true },
                                   { responseType: 'arraybuffer' });
      const buf = Buffer.from(r.data);
      if (!buf.length) return null;
      return { buf, png: tipo.includes('png') || buf.slice(0, 4).toString('hex') === '89504e47' };
    } catch (e) {
      // Si la cuenta de servicio no la puede abrir, se intenta como archivo
      // público antes de rendirse: hay fotos compartidas con "cualquiera con
      // la liga" que sí bajan por ahí.
      try {
        const r = await fetch('https://drive.google.com/uc?export=download&id=' + id,
                              { redirect: 'follow' });
        if (!r.ok) return null;
        const tipo = (r.headers.get('content-type') || '').toLowerCase();
        if (tipo.includes('html') || tipo.includes('webp') || tipo.includes('svg')) return null;
  ...
  try {
    ...
    if (!r.ok) return null;
    const tipo = (r.headers.get('content-type') || '').toLowerCase();
    // Una página HTML no es una foto: es lo que devuelve una liga de compartir
    if (tipo.includes('html') || tipo.includes('webp') || tipo.includes('svg')) return null;
    const buf = Buffer.from(await r.arrayBuffer());
    if (!buf.length) return null;
    return { buf, png: tipo.includes('png') || buf.slice(0, 4).toString('hex') === '89504e47' };
...
async function generarCotizacion(cot, opciones) {
  ...
  for (const it of cot.items) {
    // Si la foto vino de la carpeta, se baja por su id; si no, por la liga
    const clave = it.fotoDrive ? ('drive:' + it.fotoDrive) : it.foto;
    if (!clave || imgs[clave] !== undefined) continue;
    const d = it.fotoDrive
      ? await bajarImagen('https://drive.google.com/file/d/' + it.fotoDrive + '/view')
      : await bajarImagen(it.foto);
    let emb = null;
  ...
  function nuevaPagina() {
    ...
    y -= 22;
    if (pagina === 1) {
      // El proyecto va al lado del cliente, en el mismo renglón
      const lineaCliente = 'CLIENTE: ' + (cot.cliente || '');
      page.drawText(limpio(lineaCliente), { x: M, y, size: 9.5, font: reg, color: TINTA });
      if (cot.proyecto) {
        const anchoCliente = reg.widthOfTextAtSize(limpio(lineaCliente), 9.5);
        page.drawText(limpio('· PROYECTO: ' + cot.proyecto),
          { x: M + anchoCliente + 10, y, size: 9.5, font: reg, color: TINTA });
      }
      if (cot.despacho) {
  ...
  cot.items.forEach((it, idx) => {
    const emb = imgs[it.fotoDrive ? ('drive:' + it.fotoDrive) : it.foto] || null;
    const anchoTxt = 118;
    const lineas = [];
  ...
  const filas = [];
  if (cot.envio || cot.cpDestino) {
    filas.push([cot.cpDestino ? 'Envio · C.P. ' + cot.cpDestino : 'Envio',
                cot.envio ? money(cot.envio) : 'Incluido', false]);
  }
  filas.push(['TOTAL', money(cot.total), true]);
  if (!cot.entregaInmediata) {
    filas.push(['Anticipo 60%', money(cot.anticipo), true]);
    filas.push(['Finiquito 40%', money(cot.finiquito), true]);
  } else {
    filas.push(['Pago completo · entrega inmediata', '', false]);
  }
...
// bajarImagen se comparte con el formato nuevo: es la misma foto del catálogo y
// no tiene caso tener dos formas de bajarla.
module.exports = { generarCotizacion, bajarImagen };
