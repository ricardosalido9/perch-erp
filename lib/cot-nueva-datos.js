  ...
  for (const it of items) {
    ...
    productos.push({
      ...
      descripcion: (function () {
        ...
        mete(it.producto);
        // La descripción del catálogo suele empezar repitiendo el nombre —"Atril
        // Bisbita Grande . Elaborado en madera..."— y quedaba dos veces. Se quita
        // esa repetición del principio, no del texto entero: si el nombre vuelve a
        // salir a media frase, ahí sí tiene sentido.
        const quitaNombre = (t) => {
          const v = String(t == null ? '' : t).trim();
          const n = String(it.producto || '').trim();
          if (!v || !n) return v;
          const limpio2 = (x) => x.toLowerCase().normalize('NFD')
            .replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, ' ');
          if (limpio2(v).indexOf(limpio2(n)) !== 0) return v;
          return v.slice(n.length).replace(/^\s*[.·,:-]?\s*/, '');
        };
        mete(it.material);
        mete(it.desc);
