  ...
  function hojaDeCfdis(b, titulo2, conRetenciones) {
    ...
    const izq = [
      [titulo2 === 'CFDIs Emitidos' ? 'IVA Trasladado' : 'IVA Acreditable',
       money(b.totales.ivaDelPeriodo != null ? b.totales.ivaDelPeriodo : b.totales.iva), true]
    ];
    // De dónde sale: el IVA de las PUE más el de los complementos cobrados en el mes.
    // El IVA de una factura en parcialidades no se causa al facturar sino al cobrar.
    if (b.totales.ivaPUE != null) {
      izq.push(['   de facturas PUE', money(b.totales.ivaPUE), false]);
      izq.push(['   de complementos del mes (' +
                ((b.totales.complementosDelMes || {}).n || 0) + ')',
                money(b.totales.ivaComplementos), false]);
      izq.push(['IVA de las facturas emitidas', money(b.totales.iva), false]);
    }
    if (conRetenciones) {
