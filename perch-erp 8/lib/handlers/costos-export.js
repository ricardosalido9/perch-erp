...
module.exports = async (req, res) => {
  try {
    ...
    // Las comparativas se escriben en el archivo consolidado, no en el de captura diaria
    ...
    const cab = ['Folio', 'Fecha', 'Año', 'Cliente', 'Piezas', 'Proveedores',
      ...
      'Piezas estimadas', 'Piezas sin costear', 'Piezas del pedido sin salida',
      'De dónde salieron los costos', 'Renglones', 'Qué corregir'];
    const filas = [cab];
    F.forEach(r => {
      ...
      filas.push([r.folio, r.fecha, fmt(r.anio), r.cliente, r.piezas, r.proveedores,
        ...
        r.aproximados || 0, r.sinCostear || 0, r.faltanSalidas || 0,
        fuentes, (r.detalle || []).length, '']);
    });

    const cab2 = ['Folio', 'Fecha', 'Cliente', 'Pieza', 'Material', 'Proveedor',
      'Pedido usado', 'Cantidad', 'Costo unitario', 'Importe', 'De dónde salió el costo',
      'Costo catálogo', 'En Pedidos se llama', 'Costo que entraría'];
    const det = [cab2];
    F.forEach(r => {
      (r.detalle || []).forEach(p => {
        det.push([r.folio, r.fecha, r.cliente, p.producto, p.material, p.proveedor,
          ...
          p.sugerencia ? p.sugerencia.producto : '', p.sugerencia ? p.sugerencia.importe : '']);
      });
    });
    ...
    F.forEach(r => {
      ...
      Object.keys(porProd).forEach(k => {
        ...
        let motivo = '';
        if (r.malNombre && d.piezas.length && !d.costoR) motivo = 'El producto está escrito distinto en Ventas y Pedidos';
        else if (!d.prod && r.faltanSalidas) motivo = 'Falta registrar la salida de estas piezas';
        else if (!d.prod) motivo = 'Vendido pero sin piezas en producción';
        ...
        if (motivo !== 'Cuadra') {
          const m = motivoFolio[r.folio] = motivoFolio[r.folio] || {};
          m[motivo] = (m[motivo] || 0) + Math.abs(delta);
        }
      });
    });

    // Se marca en cada folio cuál es el motivo que más pesa
    const iMot = cab.indexOf('Qué corregir');
    for (let i = 1; i < filas.length; i++) {
      const m = motivoFolio[filas[i][0]];
      if (!m) { filas[i][iMot] = 'Cuadra'; continue; }
      const orden = Object.keys(m).sort((x, y) => m[y] - m[x]);
      filas[i][iMot] = orden[0] + (orden.length > 1 ? ' (+' + (orden.length - 1) + ' más)' : '');
    }
