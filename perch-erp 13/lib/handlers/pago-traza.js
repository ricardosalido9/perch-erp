...
module.exports = async (req, res) => {
  try {
    ...
    if (eP) {
      ...
      const b = normPedido(buscado);
      salida.pagosParecidos = eg.rows.filter(r => {
        const v = normPedido(r[eP]);
        if (!v || v === b) return false;
        // Se comparan quitando guiones y ceros a la izquierda, para cazar variantes
        const sin = (x) => x.replace(/[^a-z0-9]/g, '');
        return v.indexOf(b) !== -1 || b.indexOf(v) !== -1 || sin(v) === sin(b);
      }).slice(0, 10).map(r => ({ fila: r._fila, pedidoCrudo: JSON.stringify(txt(r[eP])),
                                  monto: num(r[eT]), proveedor: txt(ePr ? r[ePr] : '') }));

      // Si se da un monto o un proveedor, se busca por ahí: así aparece el pago aunque
      // la columna Pedido esté vacía o escrita distinto.
      const montoBuscado = num(q.monto || '');
      const provBuscado = norm(q.proveedor || '');
      if (montoBuscado) {
        salida.porMonto = eg.rows.filter(r => Math.abs(num(r[eT]) - montoBuscado) <= 1)
          .slice(0, 15).map(r => ({
            fila: r._fila, fecha: txt(r[eF]), monto: num(r[eT]),
            proveedor: txt(ePr ? r[ePr] : ''), pedidoCrudo: JSON.stringify(txt(r[eP])),
            descripcion: txt(eD ? r[eD] : '')
          }));
      }
      if (provBuscado) {
        const suyos = eg.rows.filter(r => norm(ePr ? r[ePr] : '') === provBuscado);
        salida.delProveedor = {
          total: suyos.length,
          conPedido: suyos.filter(r => normPedido(r[eP])).length,
          sinPedido: suyos.filter(r => !normPedido(r[eP])).length,
          pedidosQueUsa: Array.from(new Set(suyos.map(r => txt(r[eP])).filter(Boolean))).slice(0, 25),
          ultimos: suyos.slice(-8).map(r => ({
            fila: r._fila, fecha: txt(r[eF]), monto: num(r[eT]),
            pedidoCrudo: JSON.stringify(txt(r[eP])), descripcion: txt(eD ? r[eD] : '')
          }))
        };
      }
    }
    ...
    salida.diagnostico =
      ...
      : !nEx ? 'El pedido existe en Pedidos pero EGRESOS no tiene ningún pago con ese número. ' +
               'Agrega &proveedor=NOMBRE o &monto=110084 a esta misma dirección para buscar el pago por ahí.'
